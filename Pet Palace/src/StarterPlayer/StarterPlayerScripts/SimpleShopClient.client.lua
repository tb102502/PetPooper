...
local function applyUpgradeColors(upgradeFrame, colorData, currentLevel, maxLevel)
	...
	local primaryColor = Color3.fromRGB(colorData.primary[1], colorData.primary[2], colorData.primary[3])
	local secondaryColor = Color3.fromRGB(colorData.secondary[1], colorData.secondary[2], colorData.secondary[3])
	local iconColor = Color3.fromRGB(colorData.icon[1], colorData.icon[2], colorData.icon[3])
	local accentColor = Color3.fromRGB(colorData.accent[1], colorData.accent[2], colorData.accent[3])
	...
	if purchaseButton and currentLevel < maxLevel then
		local connections = upgradeFrame:GetAttribute("HoverConnections") or {}
		...
		local hoverIn = purchaseButton.MouseEnter:Connect(function()
			purchaseButton.BackgroundColor3 = accentColor
		end)

		local hoverOut = purchaseButton.MouseLeave:Connect(function()
			purchaseButton.BackgroundColor3 = Color3.fromRGB(50, 150, 50) -- Default green
		end)

		-- Store connections for cleanup
		upgradeFrame:SetAttribute("HoverConnections", {hoverIn, hoverOut})
	end
end
			...
			if itemFrame then
				...
				if purchaseButton then
					if upgradeData.maxed then
						...
						local sellPetButton = Instance.new("TextButton")
						sellPetButton.Name = "SellPetButton"
						sellPetButton.MouseButton1Click:Connect(function()
							-- This would normally open a pet selection UI
							-- For now, let's assume we're selling the first pet
							local playerData = _G.ShopData or {}
							if playerData.pets and #playerData.pets > 0 then
								SellPet:FireServer(playerData.pets[1].id)
							end
						end)
					end
				end
...
print("Enhanced Shop Client with colors loaded!")