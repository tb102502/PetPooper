--[[
    ShopSystemClientLoader.client.lua
    Client-side loader for the ShopSystem
    Created: 2025-05-24
...
print("ShopSystemClientLoader: Starting...")

-- Create the client module if it doesn't exist
local function CreateClientModule()
	if not ReplicatedStorage:FindFirstChild("ShopSystemClient") then
		print("ShopSystemClientLoader: Creating ShopSystemClient module...")
		local clientModule = Instance.new("ModuleScript")
		clientModule.Name = "ShopSystemClient"
		clientModule.Source = [[
--[[
    ...
    Author: GitHub Copilot for tb102502
		...
		function ShopSystemClient:Initialize()
			...
			local remoteFolder = ReplicatedStorage:WaitForChild("ShopSystem", 30)
		...
		return ReplicatedStorage:FindFirstChild("ShopSystemClient")
end

-- Function to wait for the ShopSystem folder
local function waitForShopSystem()
	local startTime = tick()
	local timeout = 60 -- Wait up to 60 seconds

	print("ShopSystemClientLoader: Waiting for ShopSystem folder...")

	while tick() - startTime < timeout do
		local folder = ReplicatedStorage:FindFirstChild("ShopSystem")
		if folder then
			print("ShopSystemClientLoader: Found ShopSystem folder")
			-- Wait a bit more to ensure all remotes are created
			wait(1)
			return folder
		end
...
-- Create the client module first
local clientModule = CreateClientModule()
...
-- Wait for the ShopSystem folder
local shopSystemFolder = waitForShopSystem()
if not shopSystemFolder then
	warn("ShopSystemClientLoader: Failed to find ShopSystem folder after waiting")
	return
end

-- Try to load the module
local success, ShopSystemClient = pcall(function()
	return require(clientModule)
end)

if not success then
	warn("ShopSystemClientLoader: Failed to require ShopSystemClient module: " .. tostring(ShopSystemClient))
	return
end

-- Initialize the client system
if ShopSystemClient and typeof(ShopSystemClient) == "table" and typeof(ShopSystemClient.Initialize) == "function" then
	spawn(function()
		-- Add a delay to ensure server is ready
		wait(2)
		ShopSystemClient:Initialize()
		print("ShopSystemClient successfully initialized")
	end)
else
	warn("ShopSystemClientLoader: ShopSystemClient module doesn't have proper structure")
end

-- Make the ShopSystemClient available to other scripts
_G.ShopSystemClient = ShopSystemClient