...
local function updateInventoryUI()
	...
	for _, seed in ipairs(FarmingSeeds.Seeds) do
		...
		if quantity > 0 then
			...
			itemLabel.Text = seed.Name .. " x" .. tostring(quantity)
			...
			selectButton.Size = UDim2.new(0.3, 0, 1, 0)
			selectButton.Position = UDim2.new(0.7, 0, 0, 0)
			selectButton.BackgroundColor3 = Color3.fromRGB(100, 180, 80)
			selectButton.Text = selectedSeed == seed.ID and "Selected" or "Select"
			...
			updateInventoryUI.OnClientEvent:Connect(function(newInventory)
				playerFarmingData.inventory = newInventory
				updateInventoryUI()
			end)
		end
	end
	...
	if container:FindFirstChild("ListLayout") then
		container.CanvasSize = UDim2.new(0, 0, 0, container.ListLayout.AbsoluteContentSize.Y)
	end
...
return {
	...
	SetInventoryFrame = function(frame) inventoryFrame = frame end,
	ShowNotification = showNotification,
	SetFarmingUI = function(ui) farmingUI = ui end,
}