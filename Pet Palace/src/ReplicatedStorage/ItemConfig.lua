...
local CROP_SELLING_FIXES = {
	carrot = {sellable = true, type = "crop", category = "crops"},
	corn = {sellable = true, type = "crop", category = "crops"},
	strawberry = {sellable = true, type = "crop", category = "crops"},
	wheat = {sellable = true, type = "crop", category = "crops"},
	potato = {sellable = true, type = "crop", category = "crops"},
	tomato = {sellable = true, type = "crop", category = "crops"},
	cabbage = {sellable = true, type = "crop", category = "crops"},
	radish = {sellable = true, type = "crop", category = "crops"},
	broccoli = {sellable = true, type = "crop", category = "crops"}
}

function ItemConfig.GetCropSellPrice(cropId)
	local crop = ItemConfig.Crops[cropId]
	if crop and crop.sellValue then
		return crop.sellValue
	end

	local shopItem = ItemConfig.ShopItems[cropId]
	if shopItem and shopItem.price then
		return math.floor(shopItem.price * 0.5) -- 50% of shop price
	end
...
function ItemConfig.IsCropSellable(cropId)
	local crop = ItemConfig.Crops[cropId]
	if crop then
		return crop.sellable ~= false and not crop.notSellable
	end
	return false
end

print("ItemConfig: ✅ Crop and crafting fixes loaded!")
print("🌾 Crops now have selling properties")
print("🔨 Crafting items have proper categories")
print("💰 Selling system should work properly")
-- ========== MINING SYSTEM DATA ==========
...
local CROP_SHOP_ENTRIES = {
	-- These are not purchasable, only sellable
	carrot = {
		id = "carrot",
		name = "🥕 Carrot",
		price = 8,
		currency = "coins",
		category = "crops",
		description = "Fresh orange carrot. (Selling only)",
		icon = "🥕",
		type = "crop",
		maxQuantity = 999,
		notPurchasable = true,
	...
	corn = {
		...
		currency = "coins",
		category = "crops",
		description = "Sweet yellow corn. (Selling only)",
		icon = "🌽",
		type = "crop",
		maxQuantity = 999,
		notPurchasable = true,
	...
	strawberry = {
		...
		currency = "coins",
		category = "crops",
		description = "Sweet red strawberry. (Selling only)",
		icon = "🍓",
		type = "crop",
		maxQuantity = 999,
		notPurchasable = true,
	...
	wheat = {
		...
		currency = "coins",
		category = "crops",
		description = "Golden wheat grain. (Selling only)",
		icon = "🌾",
		type = "crop",
		maxQuantity = 999,
		notPurchasable = true,
	...
	potato = {
		...
		currency = "coins",
		category = "crops",
		description = "Hearty potato. (Selling only)",
		icon = "🥔",
		type = "crop",
		maxQuantity = 999,
		notPurchasable = true,
	...
	tomato = {
		...
		currency = "coins",
		category = "crops",
		description = "Juicy red tomato. (Selling only)",
		icon = "🍅",
		type = "crop",
		maxQuantity = 999,
		notPurchasable = true,
	...
	cabbage = {
		...
		currency = "coins",
		category = "crops",
		description = "Fresh leafy cabbage. (Selling only)",
		icon = "🥬",
		type = "crop",
		maxQuantity = 999,
		notPurchasable = true,
	...
	radish = {
		...
		currency = "coins",
		category = "crops",
		description = "Spicy radish. (Selling only)",
		icon = "🌶️",
		type = "crop",
		maxQuantity = 999,
		notPurchasable = true,
	...
	broccoli = {
		...
		currency = "coins",
		category = "crops",
		description = "Nutritious green broccoli. (Selling only)",
		icon = "🥦",
		type = "crop",
		maxQuantity = 999,
		notPurchasable = true,
...
-- ========== CROP DATA ==========
	ItemConfig.Crops = {
		carrot = { 
			...
			icon = "🥕", 
			rarity = "common",
			sellable = true, -- IMPORTANT: Add this
			price = 8 -- IMPORTANT: Add this for sell price calculation
		},
		corn = { 
			id = "corn", 
			name = "🌽 Corn", 
		...
		golden_fruit = { 
			...
			sellValue = 1000, -- Make it valuable
			sellCurrency = "farmTokens", -- Sell for premium currency
			feedValue = 10, 
			cropPoints = 10, 
			icon = "✨", 
			rarity = "legendary",
			sellable = true, -- IMPORTANT: Add this
			price = 1000 -- IMPORTANT: Add this
		},
		glorious_sunflower = { 
			...
			type = "crop", 
			category = "crops", 
			description = "🏆 LEGENDARY PREMIUM CROP 🏆 - Display piece, not for sale!",
			sellValue = 0, 
			sellCurrency = "farmTokens", 
			feedValue = 0, 
			cropPoints = 0, 
			icon = "🌻", 
			rarity = "divine",
			sellable = false, -- This one should NOT be sellable
			notSellable = true -- Explicitly mark as not sellable
		}
	}
-- ========== COMPLETE SHOP ITEMS ==========
ItemConfig.ShopItems = {
	...
	milk = {
		id = "milk",
		name = "🥛 Fresh Milk",
		price = 10, -- Base price for sell value calculation
		currency = "coins",
		category = "livestock", -- Make sure this matches your sellable categories
		description = "Fresh milk collected from your cows. Can be sold for coins.",
		icon = "🥛",
		type = "material", -- Important: This should be in your sellableTypes
		maxQuantity = 999,
		notPurchasable = true, -- Can't buy milk, only sell it
		sellable = true, -- Explicitly mark as sellable
	},
	milk_efficiency_1 = {
	...
	local CRAFTING_ITEMS_FIXED = {
		basic_workbench = {
			...
			price = 50000,
			currency = "coins",
			category = "crafting", -- CRITICAL: Must be "crafting"
			icon = "🔨",
			maxQuantity = 1,
			type = "building"
		},
		...
		mystical_altar = {
			...
			price = 500, -- Fixed: was 500000, but currency is farmTokens so this is appropriate
			currency = "farmTokens", -- This should be farmTokens, not 500000 farmTokens
			category = "crafting", -- CRITICAL: Must be "crafting"
			icon = "🔮",
			maxQuantity = 1,
			type = "building",
...
game:GetService("Players").PlayerAdded:Connect(function(player)
	player.Chatted:Connect(function(message)
		if player.Name == "TommySalami311" then -- Replace with your username
			...
			if command == "/debugcow" then
				local cowType = args[2] or "silver_cow_upgrade"
				if _G.ShopSystem then
					_G.ShopSystem:DebugCowPurchase(player, cowType)
				end

			elseif command == "/debugcrafting" then
				print("=== CRAFTING ITEMS DEBUG ===")
				if _G.ShopSystem and _G.ShopSystem.ItemConfig then
					local craftingItems = 0
					for itemId, item in pairs(_G.ShopSystem.ItemConfig.ShopItems) do
						if item.category == "crafting" then
							craftingItems = craftingItems + 1
							print("🔨 " .. itemId .. ": " .. item.name .. " (" .. item.price .. " " .. item.currency .. ")")
						end
					end
					print("Total crafting items: " .. craftingItems)
				else
					print("ShopSystem or ItemConfig not available")
				end

			elseif command == "/debugselling" then
				print("=== SELLING DEBUG ===")
				local playerData = _G.GameCore and _G.GameCore:GetPlayerData(player)
				if playerData and _G.ShopSystem then
					print("Checking sellable items...")
					local sellableItems = _G.ShopSystem:GetSellableItems(playerData)
					for itemId, itemData in pairs(sellableItems) do
						print("💰 " .. itemId .. ": " .. itemData.quantity .. " units (sell for " .. itemData.sellPrice .. " " .. itemData.currency .. ")")
					end
				else
					print("PlayerData or ShopSystem not available")
				end
			...
			elseif command == "/givecrops" then
				...
				if playerData then
					...
					print("Gave 10 of each test crop to " .. player.Name)
					if _G.GameCore then
						_G.GameCore:SavePlayerData(player)
					end
				end

			elseif command == "/forcebasiccow" then
				-- Force give a basic cow for testing upgrades
				local playerData = _G.GameCore and _G.GameCore:GetPlayerData(player)
				if playerData and _G.GameCore then
					-- Initialize livestock if needed
					playerData.livestock = playerData.livestock or {cows = {}}
					playerData.livestock.cows = playerData.livestock.cows or {}

					-- Add a basic cow
					local cowId = "test_basic_cow_" .. os.time()
					...
					_G.GameCore:SavePlayerData(player)
					print("Gave basic cow to " .. player.Name .. " for upgrade testing")
				end
			end
		end
	end)
end)
function ItemConfig.MergeCropsIntoShopItems()
	for cropId, cropData in pairs(CROP_SHOP_ENTRIES) do
		ItemConfig.ShopItems[cropId] = cropData
	end
	print("ItemConfig: Merged " .. #CROP_SHOP_ENTRIES .. " crop selling entries into ShopItems")
end

print("🛠️ COMPREHENSIVE FIXES LOADED!")
print("🔧 Debug Commands:")
print("  /debugcow [cowType] - Debug cow purchase issues")
print("  /debugcrafting - Check crafting items visibility")
print("  /debugselling - Check sellable items")
print("  /givecrops - Add test crops for selling")
print("  /forcebasiccow - Add basic cow for upgrade testing")

local ENHANCED_CROP_SHOP_ENTRIES = {
	carrot = {
		id = "carrot",
		name = "🥕 Carrot",
		price = 8,
		currency = "coins",
		category = "crops",
		description = "Fresh orange carrot. Sell for coins!",
		icon = "🥕",
		type = "crop",
		maxQuantity = 999,
		notPurchasable = true,
	...
	glorious_sunflower = {
		...
		currency = "farmTokens",
		category = "crops",
		description = "🏆 LEGENDARY PREMIUM CROP 🏆 - Display piece, cannot be sold!",
		icon = "🌻",
		type = "crop", 
		maxQuantity = 999,
		notPurchasable = true,
...
function ItemConfig.MergeCropsIntoShopItems()
	print("ItemConfig: 🌾 Merging enhanced crops into ShopItems...")

	if not ItemConfig.ShopItems then
		ItemConfig.ShopItems = {}
		print("ItemConfig: ⚠️ ShopItems was nil, initialized empty table")
	end

	local mergedCount = 0
	for cropId, cropData in pairs(ENHANCED_CROP_SHOP_ENTRIES) do
		ItemConfig.ShopItems[cropId] = cropData
		mergedCount = mergedCount + 1
		print("ItemConfig: ✅ Merged crop: " .. cropId .. " (sell value: " .. (cropData.sellValue or 0) .. ")")
	end

	print("ItemConfig: 🌾 Successfully merged " .. mergedCount .. " crops into ShopItems")
	return mergedCount
end

-- Enhanced crop sell price lookup with fallbacks
function ItemConfig.GetCropSellPriceEnhanced(cropId)
	print("ItemConfig: 🔍 Looking up sell price for crop: " .. cropId)

	-- Check ShopItems first (after merge)
	if ItemConfig.ShopItems and ItemConfig.ShopItems[cropId] then
		local item = ItemConfig.ShopItems[cropId]
		if item.sellValue then
			print("ItemConfig: 💰 Found sell value in ShopItems: " .. item.sellValue)
			return item.sellValue
		elseif item.price then
			local sellPrice = math.floor(item.price * 0.5) -- 50% of buy price
			print("ItemConfig: 💰 Calculated sell price from buy price: " .. sellPrice)
			return sellPrice
		end
	end

	-- Check Crops table
	if ItemConfig.Crops and ItemConfig.Crops[cropId] then
		local crop = ItemConfig.Crops[cropId]
		if crop.sellValue then
			print("ItemConfig: 💰 Found sell value in Crops: " .. crop.sellValue)
			return crop.sellValue
		elseif crop.price then
			local sellPrice = math.floor(crop.price * 0.5)
			print("ItemConfig: 💰 Calculated sell price from Crops price: " .. sellPrice)
			return sellPrice
		end
	end

	print("ItemConfig: ❌ No sell price found for: " .. cropId)
	return 1 -- Minimum fallback price
end

-- Enhanced crop sellability check
function ItemConfig.IsCropSellableEnhanced(cropId)
	print("ItemConfig: 🔍 Checking if crop is sellable: " .. cropId)

	-- Check ShopItems first
	if ItemConfig.ShopItems and ItemConfig.ShopItems[cropId] then
		local item = ItemConfig.ShopItems[cropId]
		if item.notSellable then
			print("ItemConfig: ❌ Crop marked as not sellable: " .. cropId)
			return false
		end
		if item.sellable ~= false then -- Default to true unless explicitly false
			print("ItemConfig: ✅ Crop is sellable: " .. cropId)
			return true
		end
	end
	...
	if ItemConfig.Crops and ItemConfig.Crops[cropId] then
		...
		if crop.notSellable then
			print("ItemConfig: ❌ Crop marked as not sellable in Crops: " .. cropId)
			return false
		end
		if crop.sellable ~= false then
			print("ItemConfig: ✅ Crop is sellable in Crops: " .. cropId)
			return true
		end
	end

	-- Default: most crops should be sellable
	print("ItemConfig: ⚠️ Defaulting to sellable for: " .. cropId)
	return true
end

-- CRITICAL: Call the merge function immediately when ItemConfig loads
print("ItemConfig: 🚀 Auto-calling crop merge function...")
ItemConfig.MergeCropsIntoShopItems()

-- Debug function to verify crop selling setup
function ItemConfig.DebugCropSelling()
	print("=== CROP SELLING DEBUG ===")

	if not ItemConfig.ShopItems then
		print("❌ ShopItems is nil!")
		return
	end

	local cropCount = 0
	local sellableCrops = 0

	for itemId, item in pairs(ItemConfig.ShopItems) do
		if item.category == "crops" then
			cropCount = cropCount + 1
			print("🌾 Crop: " .. itemId)
			print("  Name: " .. (item.name or "N/A"))
			print("  Price: " .. (item.price or "N/A")) 
			print("  Sellable: " .. tostring(item.sellable ~= false))
			print("  Not Sellable: " .. tostring(item.notSellable or false))
			print("  Sell Value: " .. (item.sellValue or "N/A"))

			if item.sellable ~= false and not item.notSellable then
				sellableCrops = sellableCrops + 1
			end
		end
	end

	print("Total crops in ShopItems: " .. cropCount)
	print("Sellable crops: " .. sellableCrops)
	print("==========================")
end
...
game:GetService("Players").PlayerAdded:Connect(function(player)
	player.Chatted:Connect(function(message)
		if player.Name == "TommySalami311" then -- Replace with your username
			if message:lower() == "/debugcropselling" then
				ItemConfig.DebugCropSelling()
			elseif message:lower() == "/remergecrops" then
				local count = ItemConfig.MergeCropsIntoShopItems()
				print("Re-merged " .. count .. " crops")
			end
		end
	end)
end)

print("ItemConfig: ✅ Enhanced crop selling system loaded!")
print("🔧 Debug Commands:")
print("  /debugcropselling - Check crop selling setup")
print("  /remergecrops - Re-merge crops into ShopItems")
return ItemConfig