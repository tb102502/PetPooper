...
ItemConfig.ShopItems = {
	...
	-- Add this farm plot expansion item to your ItemConfig.lua ShopItems table:
	...
	farm_plot_expansion = {
		...
		type = "farmPlot",
		category = "farm",
		price = 200, -- Base price for additional plots
		currency = "coins",
		description = "Expand your farm with an additional plot! More space for more crops.",
		maxQuantity = 9, -- Can buy up to 9 additional plots (total 10 with starter)
		requiresPurchase = "farm_plot_starter", -- Must have starter plot first
		icon = "🌾",
		priceMultiplier = 1.5, -- Each additional plot costs 50% more
		effects = {
			expandFarm = true
		}
	},
	-- ADD THESE ITEMS to your ItemConfig.lua ShopItems table
	-- IMPORTANT: Make sure category = "farm" (not "farming")

	-- ========== FARM PROTECTION UPGRADES ==========
	basic_roof = {
		id = "basic_roof",
		name = "🏠 Basic Roof Protection",
		type = "farmUpgrade",
		category = "farm", -- FIXED: Changed from "farming" to "farm"
		price = 200,
		currency = "coins",
		description = "Protect your crops from UFO attacks! Basic roof covers 1 farm plot.",
		requiresPurchase = "farm_plot_starter", -- Must have farm plot first
		icon = "🏠",
		maxQuantity = 10, -- Can buy up to 10 roofs (one per plot)
		effects = {
			protectsFromUFO = true,
			coverage = 1 -- Covers 1 plot
		}
	},
	...
	reinforced_roof = {
		...
		requiresPurchase = "basic_roof", -- Must have basic roof first
		icon = "🏘️",
		maxQuantity = 3, -- Can buy up to 3 (covers 12 plots total)
		effects = {
			protectsFromUFO = true,
			coverage = 4 -- Covers 4 plots
		}
	},
	...
	mega_dome = {
		...
		requiresPurchase = "reinforced_roof", -- Must have reinforced roof first
		icon = "🛡️",
...
function ItemConfig.CanPlayerBuy(itemId, playerData)
	...
	-- Check purchase requirements (for farm expansion)
	if item.requiresPurchase then
		if not playerData.purchaseHistory or not playerData.purchaseHistory[item.requiresPurchase] then
			return false, "Requires " .. item.requiresPurchase
		end
	end
	...
	-- Check if player already has maximum plots for expansion
	if item.id == "farm_plot_expansion" then
		local currentPlots = playerData.farming and playerData.farming.plots or 0
		if currentPlots >= 10 then
			return false, "Maximum farm plots reached (10)"
		end
	end

	-- Calculate cost (with scaling for multiple purchases)
