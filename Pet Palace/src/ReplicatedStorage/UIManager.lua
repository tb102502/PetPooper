...
function UIManager:StyleScrollBars()
	print("🎨 UIManager: Applying scroll bar styling with safety checks...")

	-- Apply consistent scroll bar styling across all shop scroll frames
	local function styleScrollFrameSafe(scrollFrame, frameName)
		if not scrollFrame then
			warn("UIManager: StyleScrollBars - " .. frameName .. " is nil")
			return false
		end

		if not scrollFrame:IsA("ScrollingFrame") then
			warn("UIManager: StyleScrollBars - " .. frameName .. " is not a ScrollingFrame")
			return false
		end

		local success = pcall(function()
			-- Safe property setting with existence checks
			if scrollFrame.ScrollBarThickness then
				scrollFrame.ScrollBarThickness = 12
			end

			if scrollFrame.ScrollBarImageColor3 then
				scrollFrame.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)
			end

			if scrollFrame.ScrollBarImageTransparency then
				scrollFrame.ScrollBarImageTransparency = 0.2
			end

			if scrollFrame.BorderSizePixel then
				scrollFrame.BorderSizePixel = 0
			end

			-- Only set scroll bar images if the properties exist
			local success1 = pcall(function()
				if scrollFrame.BottomImage then
					scrollFrame.BottomImage = "rbxasset://textures/ui/Scroll/scroll-bottom.png"
				end
			end)

			local success2 = pcall(function()
				if scrollFrame.TopImage then
					scrollFrame.TopImage = "rbxasset://textures/ui/Scroll/scroll-top.png"
				end
			end)

			local success3 = pcall(function()
				if scrollFrame.MidImage then
					scrollFrame.MidImage = "rbxasset://textures/ui/Scroll/scroll-middle.png"
				end
			end)

			if not (success1 and success2 and success3) then
				print("UIManager: Some scroll bar images could not be set for " .. frameName)
			end
		end)

		if success then
			print("UIManager: Successfully styled scroll frame: " .. frameName)
			return true
		else
			warn("UIManager: Failed to style scroll frame: " .. frameName)
			return false
		end
	end

	-- Style all shop scroll frames with enhanced safety
	if self.State.Menus and self.State.Menus.Shop then
		local shopMenu = self.State.Menus.Shop

		-- Get content area safely
		local contentArea = shopMenu:FindFirstChild("ContentArea")
		if not contentArea then
			warn("UIManager: Shop ContentArea not found")
			return
		end

		-- Tab scroll frame styling
		local tabHeader = contentArea:FindFirstChild("TabHeader")
		if tabHeader then
			local tabScroll = tabHeader:FindFirstChild("TabScroll")
			if tabScroll then
				styleScrollFrameSafe(tabScroll, "TabScroll")
			else
				warn("UIManager: TabScroll not found in TabHeader")
			end
		else
			warn("UIManager: TabHeader not found in ContentArea")
		end

		-- Item content scroll frame styling
		local itemContent = contentArea:FindFirstChild("ItemContent")
		if itemContent then
			styleScrollFrameSafe(itemContent, "ItemContent")
		else
			warn("UIManager: ItemContent not found in ContentArea")
		end

		-- Style any other scroll frames in the shop
		local function styleAllScrollFramesRecursive(parent, parentName)
			for _, child in ipairs(parent:GetChildren()) do
				if child:IsA("ScrollingFrame") then
					styleScrollFrameSafe(child, parentName .. "." .. child.Name)
				elseif child:IsA("GuiObject") then
					styleAllScrollFramesRecursive(child, parentName .. "." .. child.Name)
				end
			end
		end

		styleAllScrollFramesRecursive(shopMenu, "ShopMenu")

	else
		warn("UIManager: Shop menu not available for styling")
	end

	-- Style scroll frames in other menus too
	local menusToStyle = {"Farm", "Mining", "Crafting", "Settings"}
	for _, menuName in ipairs(menusToStyle) do
		if self.State.Menus and self.State.Menus[menuName] then
			local menu = self.State.Menus[menuName]
			local contentArea = menu:FindFirstChild("ContentArea")
			if contentArea and contentArea:IsA("ScrollingFrame") then
				styleScrollFrameSafe(contentArea, menuName .. "ContentArea")
			end
		end
	end

	print("🎨 UIManager: Scroll bar styling completed with safety checks")
end

-- ========== ENHANCED ERROR HANDLING FOR SCROLL FRAME OPERATIONS ==========

function UIManager:SafeScrollFrameOperation(scrollFrame, operation, frameName)
	frameName = frameName or "Unknown"

	if not scrollFrame then
		warn("UIManager: SafeScrollFrameOperation - " .. frameName .. " is nil")
		return false
	end

	if not scrollFrame:IsA("ScrollingFrame") then
		warn("UIManager: SafeScrollFrameOperation - " .. frameName .. " is not a ScrollingFrame")
		return false
	end

	local success, result = pcall(operation, scrollFrame)
	if not success then
		warn("UIManager: SafeScrollFrameOperation failed for " .. frameName .. ": " .. tostring(result))
		return false
	end
...
-- ========== ENHANCED SCROLL FRAME UTILITIES ==========

function UIManager:ValidateScrollFrame(scrollFrame, frameName)
	frameName = frameName or "Unknown"

	local issues = {}

	if not scrollFrame then
		table.insert(issues, "ScrollFrame is nil")
		return false, issues
	end

	if not scrollFrame:IsA("ScrollingFrame") then
		table.insert(issues, "Not a ScrollingFrame instance")
		return false, issues
	end

	if not scrollFrame.Parent then
		table.insert(issues, "ScrollFrame has no parent")
	end

	-- Check for essential properties
	local requiredProperties = {
		"CanvasSize", "Size", "ScrollBarThickness", "ScrollingDirection"
	}

	for _, prop in ipairs(requiredProperties) do
		if not pcall(function() return scrollFrame[prop] end) then
			table.insert(issues, "Missing property: " .. prop)
		end
	end

	local isValid = #issues == 0
	if not isValid then
		warn("UIManager: ScrollFrame validation failed for " .. frameName .. ":")
...
function UIManager:GetAllScrollFrames()
	local scrollFrames = {}

	local function findScrollFramesRecursive(parent, path)
		path = path or ""
		...
		for _, child in ipairs(parent:GetChildren()) do
			...
			if child:IsA("ScrollingFrame") then
				table.insert(scrollFrames, {
					frame = child,
					path = childPath,
					parent = parent
				})
			elseif child:IsA("GuiObject") then
				findScrollFramesRecursive(child, childPath)
			end
		end
	end
	...
	if self.State.MainUI then
		findScrollFramesRecursive(self.State.MainUI, "MainUI")
	end

	-- Search in individual menu UIs
	if self.State.FarmingUI then
		findScrollFramesRecursive(self.State.FarmingUI, "FarmingUI")
	end

	if self.State.MiningUI then
		findScrollFramesRecursive(self.State.MiningUI, "MiningUI")
	end

	return scrollFrames
end

function UIManager:DiagnoseScrollFrameIssues()
	print("🔍 UIManager: Diagnosing scroll frame issues...")

	local allScrollFrames = self:GetAllScrollFrames()
	print("Found " .. #allScrollFrames .. " scroll frames")

	local validFrames = 0
	local invalidFrames = 0

	for i, frameInfo in ipairs(allScrollFrames) do
		local isValid, issues = self:ValidateScrollFrame(frameInfo.frame, frameInfo.path)
		...
		if isValid then
			...
			print("❌ " .. frameInfo.path .. " - Issues: " .. table.concat(issues, ", "))
		end
	end

	print("🔍 Diagnosis complete:")
	print("  ✅ Valid frames: " .. validFrames)
	print("  ❌ Invalid frames: " .. invalidFrames)

	return validFrames, invalidFrames
end

-- ========== DEBUG COMMANDS FOR SCROLL FRAME ISSUES ==========

_G.DiagnoseScrollFrames = function()
	if _G.UIManager then
		return _G.UIManager:DiagnoseScrollFrameIssues()
	else
		print("UIManager not available")
	end
end

_G.StyleScrollBarsSafe = function()
	if _G.UIManager then
		_G.UIManager:StyleScrollBars()
		print("✅ Safe scroll bar styling completed")
	else
		print("UIManager not available")
	end
end

print("UIManager: ✅ Enhanced scroll bar styling with safety checks loaded!")
print("🔧 NEW FEATURES:")
print("  🛡️ Safe property access with existence checks")
print("  ⚠️ Enhanced error handling for missing properties")
print("  🔍 Scroll frame validation and diagnosis")
print("  🎯 Recursive scroll frame discovery")
print("")
print("🎮 Debug Commands:")
print("  _G.DiagnoseScrollFrames() - Check all scroll frames")
print("  _G.StyleScrollBarsSafe() - Apply safe styling")
