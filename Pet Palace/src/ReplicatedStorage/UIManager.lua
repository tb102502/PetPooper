...
function UIManager:RefreshShopMenu()
	...
	print("🛒 UIManager: Refreshing shop menu...")
	...
	if self.GameClient and self.GameClient.GetShopItems then
		...
		-- Debug shop items structure
		if shopItems and #shopItems > 0 then
			print("🛒 UIManager: First item structure:")
			local firstItem = shopItems[1]
			for key, value in pairs(firstItem) do
				print("  " .. key .. ": " .. tostring(value))
			end
		else
			warn("🛒 UIManager: No shop items received!")
			-- Try to debug the connection
			if self.GameClient.DebugShopConnection then
				self.GameClient:DebugShopConnection()
			end
		end
	else
		warn("🛒 UIManager: GameClient or GetShopItems not available")
		print("🛒 UIManager: GameClient exists:", self.GameClient ~= nil)
		if self.GameClient then
			print("🛒 UIManager: GetShopItems exists:", type(self.GameClient.GetShopItems) == "function")
		end
	...
	print("🛒 UIManager: Enhanced shop menu created - defaulting to seeds")
end
...
function UIManager:PopulateShopItems(categoryId)
	...
	if #categoryItems == 0 then
		print("🛒 UIManager: No items found for " .. categoryId .. ". Available categories:")
		local availableCategories = {}
		for _, item in ipairs(self.State.ShopItems) do
			if not availableCategories[item.category] then
				availableCategories[item.category] = 0
			end
			availableCategories[item.category] = availableCategories[item.category] + 1
		end

		for category, count in pairs(availableCategories) do
	...
	if gridLayout then
		...
		print("🛒 UIManager: Set canvas size to: " .. contentSize.Y + 30)
	end

	if #categoryItems == 0 then
		print("🛒 UIManager: WARNING - No items found for category: " .. categoryId)

		-- Create "no items" message with more detail
