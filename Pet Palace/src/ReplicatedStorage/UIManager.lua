...
UIManager.Config = {
	...
	shopCategories = {
		{id = "seeds", name = "🌱 Seeds", icon = "🌱", color = Color3.fromRGB(80, 150, 80)},
...
function UIManager:SetupMiningUI()
	...
	print("UIManager: Mining and crafting UI created with uniform fonts")
end
...
function UIManager:RefreshShopMenu()
	...
	-- Create item content area
	local itemContent = self:CreateShopItemContent(contentArea)

	-- Populate with initial tab content
	self:SwitchShopTab(self.State.CurrentShopTab)

	print("UIManager: Enhanced tabbed shop menu created with uniform fonts")
end
...
function UIManager:CreateShopTabHeader(parent)
	...
	-- Create scrolling frame for tabs (mobile-friendly)
	...
	for _, button in pairs(self.State.ShopTabButtons) do
		totalWidth = totalWidth + button.AbsoluteSize.X + 5
	end
...
function UIManager:CreateShopTabButton(parent, category, layoutOrder)
	...
	tabButton.Size = UDim2.new(0, 140, 1, -10)
	tabButton.BackgroundColor3 = Color3.fromRGB(0, 255, 127) -- Fixed: More visible color
	tabButton.BorderSizePixel = 0
	tabButton.Text = category.name
	tabButton.TextColor3 = Color3.fromRGB(255, 255, 255) -- Fixed: Better contrast
	...
	tabButton.MouseEnter:Connect(function()
		if self.State.CurrentShopTab ~= category.id then
			TweenService:Create(tabButton, self.Config.animations.hover, {
				BackgroundColor3 = Color3.fromRGB(80, 90, 100) -- Fixed: Lighter hover color
			}):Play()
		end
	end)
	...
	tabButton.MouseLeave:Connect(function()
		if self.State.CurrentShopTab ~= category.id then
			TweenService:Create(tabButton, self.Config.animations.hover, {
				BackgroundColor3 = Color3.fromRGB(60, 70, 80) -- Fixed: Back to base color
			}):Play()
		end
	end)
...
function UIManager:SwitchShopTab(tabId)
	if self.State.CurrentShopTab == tabId then return end
	...
	for categoryId, button in pairs(self.State.ShopTabButtons or {}) do
		if categoryId == tabId then
			-- Active tab - FIXED: Better colors
			...
			-- Inactive tab - FIXED: Better colors
			TweenService:Create(button, self.Config.animations.tabSwitch, {
	...
	print("UIManager: Switched to shop tab:", tabId)
end
...
function UIManager:PopulateShopItems(categoryId)
	...
	-- FIXED: Handle sell category completely separately
	if categoryId == "sell" then
		self:PopulateSellItems(itemContent)
		return
	end

	-- FIXED: Only handle normal shop items for non-sell categories
	local categoryItems = {}
	for _, item in ipairs(self.State.ShopItems) do
		if item.category == categoryId and item.category ~= "sell" then -- Exclude sell items
			table.insert(categoryItems, item)
		end
	end

	-- Create item cards (normal purchase items only)
...
function UIManager:RefreshStatsMenu()
	...
	-- Currency Stats
	local currencyFrame = self:CreateStatsSection(contentArea, "💰 Currency", 1)
	self:CreateCurrencyStats(currencyFrame, playerData)

	-- Farming Stats
	local farmingFrame = self:CreateStatsSection(contentArea, "🌾 Farming", 2)
	self:CreateFarmingStats(farmingFrame, playerData.farming or {})

	-- General Stats
	local generalFrame = self:CreateStatsSection(contentArea, "📊 General", 3)
	self:CreateGeneralStats(generalFrame, playerData)

	contentArea.CanvasSize = UDim2.new(0, 0, 0, 500)
end

function UIManager:CreateStatsSection(parent, title, layoutOrder)
	local sectionFrame = Instance.new("Frame")
...
function UIManager:CreateCurrencyStats(parent, playerData)
	local statsText = string.format(
		"Coins: %s\nFarm Tokens: %s\nTotal Earned: %s coins\nPurchases Made: %d",
		self:FormatNumber(playerData.coins or 0),
		self:FormatNumber(playerData.farmTokens or 0),
		self:FormatNumber(playerData.totalEarned or 0),
		playerData.totalPurchases or 0
	)
...
function UIManager:CreateFarmingStats(parent, farmingData)
	local statsText = string.format(
		"Crops Harvested: %d\nSeeds Planted: %d\nFarm Plots: %d\nFarming Level: %d",
		farmingData.cropsHarvested or 0,
		farmingData.seedsPlanted or 0,
		farmingData.plots or 0,
		farmingData.level or 1
	)
...
function UIManager:CreateGeneralStats(parent, playerData)
	...
	local statsText = string.format(
		"Play Time: %dh %dm\nJoin Date: %s\nSession Count: %d\nLast Login: %s",
		hours, minutes,
		playerData.joinDate or "Today",
		playerData.sessionCount or 1,
		playerData.lastLogin or "Now"
	)
