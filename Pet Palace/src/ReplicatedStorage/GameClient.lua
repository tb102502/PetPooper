--[[
    GameClient.lua - CONSOLIDATED CLIENT SYSTEM
...
function GameClient:HandlePetCollected(petData, coinsAwarded)
	...
	self:ShowNotification("Pet Collected!", petData.name .. " (+$" .. coinsAwarded .. ")", "success")
end

function GameClient:HandlePetEquipped(petId, petData)
	self:ShowNotification("Pet Equipped!", petData.name .. " is now following you!", "success")
end

function GameClient:HandleItemPurchased(item, quantity)
	if self.Sounds.purchase then
		self.Sounds.purchase:Play()
	end
	self:ShowNotification("Purchase Complete!", "Bought " .. quantity .. "x " .. item.name, "success")
end
...
function GameClient:PlayCollectionEffect(petData)
	...
	effectLabel.Text = "+" .. petData.name
	effectLabel.TextColor3 = self:GetRarityColor(petData.rarity or "Common")
