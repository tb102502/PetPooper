...
function GameClient:HandleItemPurchased(itemId, quantity, cost, currency)
	...
	-- FIXED: Refresh shop with proper canvas size update
	if self.UIState.CurrentPage == "Shop" then
		spawn(function()
			wait(0.5) -- Wait for server data update
			self:RefreshShopMenu() -- This now includes canvas size fix
		end)
	end
...
function GameClient:CreateFarmInfoSection(parent, layout, layoutOrder)
	...
	farmInfoFrame.Size = UDim2.new(1, 0, 0, 180) -- Increased height for new button
	...
	-- NEW: Harvest All Button
	local harvestAllButton = Instance.new("TextButton")
	harvestAllButton.Size = UDim2.new(1, 0, 0, 50)
	harvestAllButton.BackgroundColor3 = Color3.fromRGB(100, 180, 100)
	harvestAllButton.BorderSizePixel = 0
	harvestAllButton.Text = "🌾 HARVEST ALL READY CROPS"
	harvestAllButton.TextColor3 = Color3.new(1, 1, 1)
	harvestAllButton.TextScaled = true
	harvestAllButton.Font = Enum.Font.GothamBold
	harvestAllButton.LayoutOrder = 3
	harvestAllButton.Parent = farmContent

	local harvestCorner = Instance.new("UICorner")
	harvestCorner.CornerRadius = UDim.new(0.1, 0)
	harvestCorner.Parent = harvestAllButton

	-- Connect harvest all functionality
	harvestAllButton.MouseButton1Click:Connect(function()
		self:RequestHarvestAll()
	end)

	-- Hover effects for harvest button
	harvestAllButton.MouseEnter:Connect(function()
		TweenService:Create(harvestAllButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(120, 200, 120)}):Play()
	end)

	harvestAllButton.MouseLeave:Connect(function()
		TweenService:Create(harvestAllButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(100, 180, 100)}):Play()
	end)

	-- Disable button if no farm plots
	if not playerData or not playerData.farming or not playerData.farming.plots or playerData.farming.plots == 0 then
		harvestAllButton.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
		harvestAllButton.Text = "🌾 No Farm Plots Available"
		harvestAllButton.Active = false
	end
end

-- ADD this new function to GameClient.lua:

function GameClient:RequestHarvestAll()
	if not self.RemoteEvents.HarvestAllCrops then
		self:ShowNotification("System Error", "Harvest All system not available!", "error")
		return
	end

	-- Show immediate feedback
	self:ShowNotification("🌾 Harvesting...", "Checking all crops for harvest...", "info")
...
function GameClient:RefreshFarmMenu()
	...
	-- Farm Info Section (overview with harvest all button)
	self:CreateFarmInfoSection(contentArea, layout, 1)

	-- Complete Inventory Section (shows all items organized by category)
	self:CreateCompleteInventorySection(contentArea, layout, 2)

	-- Enhanced Chicken Management Section  
	...
	print("GameClient: Farm menu refreshed successfully")
end

-- ALSO ADD: Make sure CreateFarmInfoSection exists (if it's missing)

-- ALSO ADD: Make sure RequestHarvestAll exists (if it's missing)
...
function GameClient:RefreshShopMenu()
	...
	spawn(function()
		...
		if shopItems and #shopItems > 0 then
			...
			-- FIXED: Force canvas size update after content is created
			spawn(function()
				wait(0.2) -- Wait for layout to complete
				self:UpdateShopCanvasSize(contentArea)
			end)
		else
...
-- NEW: Function to properly update shop canvas size
function GameClient:UpdateShopCanvasSize(contentArea)
	if not contentArea then return end
	...
	if layout then
		...
		print("GameClient: Updated shop canvas size to", layout.AbsoluteContentSize.Y + 20)
	end
end
...
function GameClient:CreateInventoryItem(parent, itemData, layoutOrder)
	...
	itemInfo.Size = UDim2.new(0.45, 0, 1, 0)
	...
	-- ENHANCED: Different actions based on item type with "Sell All" for milk
	if itemData.id == "fresh_milk" or itemData.id == "processed_milk" or itemData.id == "cheese" then
		-- MILK PRODUCTS: Show both "Sell 1" and "Sell All" buttons

		-- Sell 1 button
		local sell1Button = Instance.new("TextButton")
		sell1Button.Size = UDim2.new(0, 70, 0, 20)
		sell1Button.Position = UDim2.new(1, -145, 0, 5)
		sell1Button.BackgroundColor3 = Color3.fromRGB(100, 100, 200)
		sell1Button.BorderSizePixel = 0
		sell1Button.Text = "💰 Sell 1"
		sell1Button.TextColor3 = Color3.new(1, 1, 1)
		sell1Button.TextScaled = true
		sell1Button.Font = Enum.Font.Gotham
		sell1Button.Parent = itemFrame

		local sell1Corner = Instance.new("UICorner")
		sell1Corner.CornerRadius = UDim.new(0.2, 0)
		sell1Corner.Parent = sell1Button

		-- Sell All button
		local sellAllButton = Instance.new("TextButton")
		sellAllButton.Size = UDim2.new(0, 70, 0, 20)
		sellAllButton.Position = UDim2.new(1, -145, 0, 27)
		sellAllButton.BackgroundColor3 = Color3.fromRGB(150, 120, 200)
		...
		sellAllCorner.CornerRadius = UDim.new(0.2, 0)
		sellAllCorner.Parent = sellAllButton

		-- Connect sell 1 functionality
		sell1Button.MouseButton1Click:Connect(function()
			self:SellMilkProduct(itemData.id, 1)
		end)

		-- Connect sell all functionality
		sellAllButton.MouseButton1Click:Connect(function()
			self:SellMilkProduct(itemData.id, itemData.quantity) -- Sell all available
		end)

	elseif itemData.id:find("egg") then
		-- EGG PRODUCTS: Also show both options
		local sell1Button = Instance.new("TextButton")
		sell1Button.Size = UDim2.new(0, 70, 0, 20)
		sell1Button.Position = UDim2.new(1, -145, 0, 5)
		sell1Button.BackgroundColor3 = Color3.fromRGB(255, 200, 100)
		sell1Button.BorderSizePixel = 0
		sell1Button.Text = "💰 Sell 1"
		sell1Button.TextColor3 = Color3.new(1, 1, 1)
		sell1Button.TextScaled = true
		...
		sellAllButton.Size = UDim2.new(0, 70, 0, 20)
		sellAllButton.Position = UDim2.new(1, -145, 0, 27)
		sellAllButton.BackgroundColor3 = Color3.fromRGB(255, 160, 100)
		sellAllButton.BorderSizePixel = 0
		sellAllButton.Text = "💰 Sell All"
		sellAllButton.TextColor3 = Color3.new(1, 1, 1)
		sellAllButton.TextScaled = true
		...
		sellAllButton.MouseButton1Click:Connect(function()
			self:SellEggProduct(itemData.id, itemData.quantity) -- Sell all available
		end)

	else
		-- OTHER ITEMS: Single action button (existing logic)
...
-- UPDATED: Enhanced milk selling with quantity support
function GameClient:SellMilkProduct(milkType, amount)
	print("GameClient: Selling " .. amount .. "x " .. milkType)

	-- Validate amount
	local playerData = self:GetPlayerData()
	if playerData and playerData.livestock and playerData.livestock.inventory then
		local available = playerData.livestock.inventory[milkType] or 0
		if available < amount then
			self:ShowNotification("Not Enough Milk", 
				"You only have " .. available .. " " .. self:GetItemDisplayName(milkType) .. "!", "error")
			return
		end
	end
	...
	if self.RemoteEvents.SellMilk then
		...
		if amount == 1 then
			self:ShowNotification("Selling Milk", "Selling " .. self:GetItemDisplayName(milkType) .. "!", "info")
		else
			self:ShowNotification("Selling All Milk", "Selling " .. amount .. "x " .. self:GetItemDisplayName(milkType) .. "!", "success")
		end
	else
...
-- UPDATED: Enhanced egg selling with quantity support  
function GameClient:SellEggProduct(eggType, amount)
	...
	if self.RemoteEvents.SellEgg then
		...
		if amount == 1 then
			self:ShowNotification("Selling Egg", "Selling " .. self:GetItemDisplayName(eggType) .. "!", "info")
		else
			self:ShowNotification("Selling All Eggs", "Selling " .. amount .. "x " .. self:GetItemDisplayName(eggType) .. "!", "success")
		end
	else
...
function GameClient:HandleEnhancedPurchase(item, button)
	print("🛒 CLIENT: Enhanced purchase attempt for " .. item.id)
	...
	-- Purchase with enhanced feedback
	self.RemoteEvents.PurchaseItem:FireServer(item.id, 1)

	-- Restore button after delay if no response
	spawn(function()
		wait(3)
...
-- Create message for when no items are available
function GameClient:CreateNoItemsMessage(parent)
	local noItemsLabel = Instance.new("TextLabel")
	noItemsLabel.Size = UDim2.new(1, 0, 1, 0)
	noItemsLabel.BackgroundTransparency = 1
	noItemsLabel.Text = "🛒 Shop Loading...\n\nIf this persists, try:\n• Closing and reopening the shop\n• Checking your internet connection"
	noItemsLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
	noItemsLabel.TextScaled = true
	noItemsLabel.Font = Enum.Font.Gotham
	noItemsLabel.Parent = parent
end

print("GameClient: ✅ Enhanced tabbed shop menu system loaded!")
print("New Features:")
print("  🏷️ Tabbed interface for better organization")
print("  🎨 Enhanced visual design with category colors")
print("  💡 Better item display with detailed information")
print("  🎯 Improved purchase feedback and states")
print("  ✨ Premium item highlighting with glow effects")
