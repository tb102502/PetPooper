...
function GameClient:SetupPestChickenEventHandlers()
	-- Pest spotted notification
	if self.RemoteEvents.PestSpotted then
		self.RemoteEvents.PestSpotted.OnClientEvent:Connect(function(pestType, cropType, plotInfo)
			self:HandlePestSpottedNotification(pestType, cropType, plotInfo)
		end)
	end

	-- Pest eliminated notification
	if self.RemoteEvents.PestEliminated then
		self.RemoteEvents.PestEliminated.OnClientEvent:Connect(function(pestType, eliminatedBy)
			self:HandlePestEliminatedNotification(pestType, eliminatedBy)
		end)
	end

	-- Chicken placement confirmation
	if self.RemoteEvents.ChickenPlaced then
		self.RemoteEvents.ChickenPlaced.OnClientEvent:Connect(function(chickenType, position)
			self:HandleChickenPlacedNotification(chickenType, position)
		end)
	end

	print("GameClient: Pest and chicken event handlers setup complete")
end
...
function GameClient:SetupCurrencyDisplay()
	...
	container.Position = UDim2.new(0.98, 0, 0.02, 0) -- Moved slightly left from 0.99 to 0.98
...
function GameClient:SetupIndividualButtons()
	...
	settingsButton.Position = UDim2.new(1, -55, 0, 10) -- Moved left by 5 pixels
...
function GameClient:RefreshFarmMenu()
	...
	-- SECTION 3: Chicken Management (NEW)
	if playerData and playerData.chickens then
		self:CreateChickenManagementSection(contentArea, layout, 3)
	end

	-- SECTION 4: Pest Control (NEW)
	if playerData and playerData.pestControl then
		self:CreatePestControlSection(contentArea, layout, 4)
	end

	-- SECTION 5: Crop Inventory and Sales
	if playerData and playerData.farming and playerData.farming.inventory then
