...
function getAllCrops()
	...
	-- Check Areas > Starter Meadow > Farm for player farms (this is where GameCore creates farms)
					...
					if playerFarm:IsA("Folder") and playerFarm.Name:find("_Farm") then
						...
						for _, plot in pairs(playerFarm:GetChildren()) do
							if plot:IsA("Model") and plot.Name:find("FarmPlot") then
								table.insert(farmPlots, plot) -- Store plot for region calculation
								...
								if plantingSpots then
									for _, spot in pairs(plantingSpots:GetChildren()) do
										if spot:IsA("Model") and spot.Name:find("PlantingSpot") then
											...
											if not isEmpty then -- Has a crop
												...
												if cropModel then
													...
													if crop then
														...
														print("UFO System: Found crop at " .. tostring(crop.Position))
													end
												end
											end
										end
	...
	-- Also check legacy locations (workspace FarmPlots)
	local farmFolder = workspace:FindFirstChild("FarmPlots") or workspace
	for _, plot in ipairs(farmFolder:GetChildren()) do
		local cropFolder = plot:FindFirstChild("Crops") -- Standard crop folder name
		if cropFolder then
...
function calculateUFODestructionRegion(farmPlots)
	if #farmPlots == 0 then
		-- Fallback to default region if no plots found
		print("UFO System: No farm plots found, using default destruction region")
		return Region3.new(Vector3.new(-450, -10, 50), Vector3.new(-300, 20, 200))
	end
	...
	for _, plot in ipairs(farmPlots) do
		if plot.PrimaryPart then
			...
			minX = math.min(minX, pos.X - 20) -- Add 20 stud buffer
	...
	local destructionRegion = Region3.new(
		Vector3.new(minX - 10, minY, minZ - 10),
		Vector3.new(maxX + 10, maxY, maxZ + 10)
	)

	print("UFO System: Calculated destruction region from " .. tostring(destructionRegion.MinPoint) .. " to " .. tostring(destructionRegion.MaxPoint))
	return destructionRegion
end

-- FIXED: More accurate crop destruction with proper protection checking
function destroyCropsInPath(pathRegion)
	local destroyed = 0
	local protected = 0
	local allCrops, farmPlots = getAllCrops()

	-- If no specific region provided, calculate one based on farm locations
	if not pathRegion then
		pathRegion = calculateUFODestructionRegion(farmPlots)
	end

	print("UFO System: Destroying crops in region " .. tostring(pathRegion.MinPoint) .. " to " .. tostring(pathRegion.MaxPoint))
	...
	for _, cropData in ipairs(allCrops) do
		...
		if crop and crop.Parent then
			...
			if cropPosition.X >= minPoint.X and cropPosition.X <= maxPoint.X and
				...
				print("UFO System: Crop at " .. tostring(cropPosition) .. " is in destruction path")

				-- Check if this crop is protected
				local isProtected = false

				-- Check protection using GameCore if available
				if _G.GameCore and plot and playerName ~= "Unknown" then
					local player = game.Players:FindFirstChild(playerName)
					if player then
						-- Get plot number from plot name
						local plotNumber = plot.Name:match("FarmPlot_(%d+)")
						if plotNumber then
							plotNumber = tonumber(plotNumber)
							isProtected = _G.GameCore:IsPlotProtected(player, plotNumber)
						end
					end
				end
				...
				if isProtected then
					...
					print("UFO System: Crop at " .. tostring(cropPosition) .. " protected by roof!")
					createProtectionEffect(crop)
					protected = protected + 1
				else
					-- Destroy the crop
					print("UFO System: Destroying unprotected crop at position " .. tostring(cropPosition))

					-- If it's part of a planting spot, reset the spot properly
					if spot then
						-- Reset spot attributes
						...
						-- Restore the green indicator for empty plot
						...
						createDestructionEffect(crop.Position)
					else
						-- Legacy crop destruction
						crop:Destroy()
						createDestructionEffect(cropPosition)
					end

					destroyed = destroyed + 1
				end
			else
				print("UFO System: Crop at " .. tostring(cropPosition) .. " is outside destruction path")
			end
		end
	end

	print("UFO System: Destroyed " .. destroyed .. " crops, " .. protected .. " crops protected")
	return destroyed, protected
end

-- ADD this new function to create destruction effects:
function createDestructionEffect(position)
	...
	fire.Name = "UFODestruction"
	fire.Size = Vector3.new(4, 8, 4)
	fire.Material = Enum.Material.Neon
	fire.Color = Color3.fromRGB(255, 100, 0) -- Orange fire
	fire.Shape = Enum.PartType.Cylinder
	fire.Anchored = true
	fire.CanCollide = false
	fire.CFrame = CFrame.new(position + Vector3.new(0, 4, 0))
	fire.Parent = workspace

	-- Animate and remove fire
	game:GetService("TweenService"):Create(fire,
...
-- UPDATED: Enhanced UFO attack loop with dynamic regions
spawn(function()
	print("UFO System: Starting enhanced UFO attack system")
	...
	while true do
		...
		-- Get all crops and farm locations
		local allCrops, farmPlots = getAllCrops()
		...
		-- Calculate dynamic destruction region based on actual farm locations
		local ufoPathRegion = calculateUFODestructionRegion(farmPlots)

		-- Warn chickens before attack (with error protection)
		...
		-- 1. Sky darkens and warning
		ufoAttackEvent:FireAllClients("START", totalCrops)

		-- 2. Wait for effect buildup
		wait(3)

		-- Scatter chickens (with error protection)
		...
		-- 3. Destroy crops using calculated region
		local destroyedCount, protectedCount = destroyCropsInPath(ufoPathRegion)
		...
		-- 5. Restore chickens (with error protection)
		spawn(function()
			wait(10)
...
-- ADD these admin commands for testing:
game.Players.PlayerAdded:Connect(function(player)
	player.Chatted:Connect(function(message)
		if player.Name == "TommySalami311" then -- Replace with your username
			...
			if command == "/testufoscan" then
				-- Test crop detection
				local crops, plots = getAllCrops()
				print("=== UFO CROP SCAN TEST ===")
				print("Found " .. #crops .. " crops")
				print("Found " .. #plots .. " farm plots")

				for i, cropData in ipairs(crops) do
					print("Crop " .. i .. ": " .. tostring(cropData.crop.Position) .. " (Owner: " .. cropData.playerName .. ")")
				end

				local region = calculateUFODestructionRegion(plots)
				print("Destruction region: " .. tostring(region.MinPoint) .. " to " .. tostring(region.MaxPoint))
				print("=========================")

			elseif command == "/testuforegion" then
				-- Show where UFO will attack
				local crops, plots = getAllCrops()
				local region = calculateUFODestructionRegion(plots)

				-- Create visual markers for the destruction region
				local minPoint = region.MinPoint
				local maxPoint = region.MaxPoint
				...
				for i, corner in ipairs(corners) do
					...
					game:GetService("Debris"):AddItem(marker, 30) -- Remove after 30 seconds
				end

				print("Admin: Created UFO destruction region markers for 30 seconds")

			elseif command == "/clearufomarkers" then
				-- Remove all UFO markers
				for _, obj in pairs(workspace:GetChildren()) do
					if obj.Name == "UFORegionMarker" then
						obj:Destroy()
					end
				end
				print("Admin: Cleared all UFO region markers")
			end
		end
	end)
end)
