...
function warnChickensOfAttack()
	...
	if not _G.ChickenSystem then return end
	...
	for playerId, chickens in pairs(_G.ChickenSystem.PlayerChickens) do
		for chickenId, chicken in pairs(chickens) do
			if chicken.type == "guinea_fowl" then
...
function scatterChickensFromUFO()
	...
	for playerId, chickens in pairs(_G.ChickenSystem.PlayerChickens) do
		for chickenId, chicken in pairs(chickens) do
			...
			local safePosition = chicken.homePosition + Vector3.new(
				math.random(-50, 50),
				0,
				math.random(-50, 50)
			)
			...
			local homeDistance = (safePosition - chicken.homePosition).Magnitude
			if homeDistance > maxDistance then
				safePosition = chicken.homePosition + (safePosition - chicken.homePosition).Unit * maxDistance
			end
			...
			-- Create panic visual effect
			createChickenPanicEffect(chicken)
		end
	end

	if scatteredCount > 0 then
		print("UFO System: Scattered " .. scatteredCount .. " chickens to safety")
	end
end
...
function checkChickenProtection(crop, playerName)
	...
	for chickenId, chicken in pairs(playerChickens) do
		...
		if not chicken.isPanicked then
			...
			local chickenData = game:GetService("ReplicatedStorage").ItemConfig.ChickenSystem.chickenTypes[chicken.type]
			if chickenData and distance <= (chickenData.huntRange or 10) then
