--[[
    ...
    FIXES:
    - Fixed CFrame/Vector3 conversion error at line 175
    - Added proper position extraction from CFrame
    - Better error handling for plot positions
]]
...
-- FIXED: Validate plot positions and fix if needed
function FarmPlotManager:ValidatePlotPositions(player, expectedPlots)
	...
	for i, plot in ipairs(plots) do
		if plot and GameCore.GetFarmPlotPosition then
			-- FIXED: GetFarmPlotPosition returns a CFrame, not Vector3
			local expectedCFrame = GameCore:GetFarmPlotPosition(player, i)
			local expectedPosition = expectedCFrame.Position -- Extract Vector3 from CFrame
			...
			if currentPosition and expectedPosition then
				...
				if distance > 5 then
					-- FIXED: Use the full CFrame, not just position
					plot:SetPrimaryPartCFrame(expectedCFrame)
...
Players.PlayerAdded:Connect(function(player)
	player.Chatted:Connect(function(message)
		...
		if player.Name == "TommySalami311" then
			...
			elseif command == "/testplotposition" then
				-- Test plot position calculation
				local plotNumber = tonumber(args[2]) or 1
				if GameCore.GetFarmPlotPosition then
					local plotCFrame = GameCore:GetFarmPlotPosition(player, plotNumber)
					local plotPosition = plotCFrame.Position
					print("Admin: Plot " .. plotNumber .. " CFrame:", plotCFrame)
					print("Admin: Plot " .. plotNumber .. " Position:", plotPosition)

					-- Teleport player to the position
					if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
						player.Character.HumanoidRootPart.CFrame = plotCFrame + Vector3.new(0, 5, 10)
						print("Admin: Teleported to plot " .. plotNumber .. " position")
					end
				else
					print("Admin: GetFarmPlotPosition method not found in GameCore")
				end
			end
		end
	end)
...
print("Features:")
print("✅ Fixed CFrame/Vector3 conversion error")
...
print("  /testplotposition [number] - Test specific plot position")
