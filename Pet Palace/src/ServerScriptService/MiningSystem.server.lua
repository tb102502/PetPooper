...
function MiningSystem:GenerateOreNodes(cave, basePosition, ownerId)
	local nodeCount = 0
	local maxNodesPerType = 25 -- Reduced to prevent overcrowding

	-- Initialize ore nodes storage for this cave
	...
	for oreType, oreData in pairs(ItemConfig.MiningSystem.ores) do
		local nodesToCreate = math.floor(maxNodesPerType * oreData.spawnChance)

		for i = 1, nodesToCreate do
			-- Use the fixed positioning method
	...
	print("MiningSystem: Generated " .. nodeCount .. " ore nodes for cave owner " .. ownerId)
end
...
function MiningSystem:FindValidOrePosition(basePosition)
	...
	while attempts < maxAttempts do
		...
		-- Simple raycast check to make sure we're not in a wall
		local raycast = workspace:Raycast(testPosition, Vector3.new(0, -10, 0))
		local validFloor = raycast and raycast.Instance and raycast.Instance.Name == "CaveFloor"

		if validFloor then
			-- Check minimum distance from cave walls (keep ore away from edges)
			local distanceFromCenter = (testPosition - basePosition).Magnitude
			if distanceFromCenter > 10 and distanceFromCenter < 45 then
				return testPosition
			end
		end
	end

	-- Final fallback - use a safe position near center
