...
function CowVisualEffects:ClearEffects(cowId)
	return self:ClearEffectsEnhanced(cowId)
end
	...
	spawn(function()
		while self.ActiveEffects[cowId] do
			...
			for i = 1, 2 do
				...
				local flash = game:GetService("TweenService"):Create(lightning,
...
function CowVisualEffects:CreateMagicalAura(cowModel, cowId)
	...
	spawn(function()
		while self.ActiveEffects[cowId] do
			...
			-- Spin and fade
			local spin = game:GetService("TweenService"):Create(rune,
				TweenInfo.new(4, Enum.EasingStyle.Linear),
				{Orientation = Vector3.new(90, 360, 0)}
			)
			local fade = game:GetService("TweenService"):Create(rune,
