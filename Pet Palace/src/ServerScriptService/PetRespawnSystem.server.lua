...
spawn(function()
	while true do
		wait(10) -- INCREASED from 3 to 10 seconds between checks
		...
		for areaName, areaData in pairs(GameCore.Systems.Pets.SpawnAreas) do
			if areaData and areaData.container and areaData.config then
				...
				local spawnInterval = areaData.config.spawnInterval or 45

				-- Only spawn if WELL below max (leave room)
				local shouldSpawn = currentPetCount < (maxPets - 1) -- Always keep 1 slot free

				-- Check if enough time has passed (much longer delays)
				local timeSinceLastSpawn = os.time() - (areaData.lastSpawn or 0)
				local canSpawn = timeSinceLastSpawn >= spawnInterval

				if shouldSpawn and canSpawn then
					-- Additional random chance - only 40% chance to spawn
