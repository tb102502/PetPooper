...
-- Add this debugging code temporarily at the start of your PetSpawner script
local function CheckPetModels()
	local petModels = ReplicatedStorage:FindFirstChild("PetModels")
	if not petModels then
		print("ERROR: PetModels folder not found in ReplicatedStorage")
		return
	end

	print("Available Pet Models:")
	for _, model in pairs(petModels:GetChildren()) do
		print("- " .. model.Name)
	end

	-- Check specifically for Corgi
	local corgiModel = petModels:FindFirstChild("Corgi")
	if corgiModel then
		print("Corgi model exists")
		-- Check if it has necessary parts
		if corgiModel:FindFirstChild("Humanoid") then
			print("Corgi has Humanoid")
		else
			print("ERROR: Corgi missing Humanoid")
		end

		if corgiModel.PrimaryPart then
			print("Corgi has PrimaryPart")
		else
			print("ERROR: Corgi missing PrimaryPart")
		end
	else
		print("ERROR: Corgi model not found")
	end
end

-- Call this function when the script starts
spawn(function()
	wait(3) -- Give time for everything to load
	CheckPetModels()
end)
...
-- Rest of your existing PetSpawner code, but replace:
-- PetInitializer.SetupPet() instead of PetMovementModule.initPet()
-- PetInitializer.CreateBasicPetModel() instead of your existing model creation
...
-- Function to create a pet at a specific position
function CreatePet(petType, spawnPosition, areaName)
	...
	if petModel.PrimaryPart then
		...
		-- Create configuration based on pet rarity
		...
		local jumpSettings = {
			jumpHeight = petType.rarity == "Legendary" and 4 or
				petType.rarity == "Epic" and 3 or
				petType.rarity == "Rare" and 2.5 or 2
		}

		-- Use PetInitializer to setup behaviors
		PetInitializer.SetupPet(petModel, movementConfig, jumpSettings)
