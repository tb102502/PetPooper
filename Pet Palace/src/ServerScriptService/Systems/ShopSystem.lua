...
function ShopSystem:ProcessCowPurchase(player, playerData, item, quantity)
	print("🐄 ShopSystem: Processing cow purchase for " .. player.Name)
	print("  Item: " .. item.id)
	...
	-- CHECK 1: Verify item has cow data
	...
	-- CHECK 2: Verify GameCore has cow configuration
	...
	-- CHECK 3: Ensure player has farm plot (auto-create if needed)
	if not playerData.farming or not playerData.farming.plots or playerData.farming.plots <= 0 then
		...
		-- Auto-create farm plot for cow purchase
		playerData.farming = playerData.farming or {}
		playerData.farming.plots = 1
		playerData.farming.inventory = playerData.farming.inventory or {}
		...
		print("✅ Player has farm plot")
	end

	-- CHECK 4: Check cow limits
	...
	if successCount > 0 then
		-- Update purchase history for single-purchase items
		if item.maxQuantity == 1 then
			playerData.purchaseHistory = playerData.purchaseHistory or {}
			playerData.purchaseHistory[item.id] = true
			print("✅ Marked as purchased in history")
		end

		return true
	else
		print("❌ No cows were successfully purchased")
		return false
	end
end

-- Also add this enhanced error handling to the main ProcessPurchase method
-- Replace the cow processing section with this:
...
function ShopSystem:ProcessPurchaseEnhanced(player, playerData, item, quantity)
	...
	print("✅ Purchase processing successful!")
