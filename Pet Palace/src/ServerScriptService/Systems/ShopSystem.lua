...
-- REPLACE your existing HandleSell method with a call to the enhanced version
function ShopSystem:HandleSell(player, itemId, quantity)
	return self:HandleSellEnhanced(player, itemId, quantity or 1)
end
...
function ShopSystem:GetShopItemByIdEnhanced(itemId)
	print("🔍 ShopSystem: Enhanced lookup for item: " .. itemId)

	if not self.ItemConfig then
		warn("ShopSystem: ItemConfig not available")
		return nil
	end

	-- First check ShopItems (where crops should be after merge)
	if self.ItemConfig.ShopItems and self.ItemConfig.ShopItems[itemId] then
		local item = self.ItemConfig.ShopItems[itemId]
		print("✅ Found item in ShopItems: " .. itemId)
		return item
	end

	-- Fallback: Check Crops table and create shop entry
	if self.ItemConfig.Crops and self.ItemConfig.Crops[itemId] then
		local crop = self.ItemConfig.Crops[itemId]
		print("⚠️ Found crop in Crops table, creating shop entry: " .. itemId)

		-- Create a shop-compatible entry from crop data
		local shopEntry = {
			id = itemId,
			name = crop.name or itemId,
			price = crop.sellValue or crop.price or 1,
			currency = crop.sellCurrency or "coins",
			category = "crops",
			description = crop.description or "Harvested crop",
			icon = crop.icon or "🌾",
			type = "crop",
			sellable = crop.sellable ~= false and not crop.notSellable,
			notPurchasable = true,
			sellValue = crop.sellValue or crop.price or 1
		}

		-- Add to ShopItems for future lookups
		if self.ItemConfig.ShopItems then
			self.ItemConfig.ShopItems[itemId] = shopEntry
			print("📦 Added crop to ShopItems: " .. itemId)
		end

		return shopEntry
	end

	print("❌ Item not found anywhere: " .. itemId)
	return nil
end

-- FIXED: Enhanced IsItemSellable with comprehensive crop detection
function ShopSystem:IsItemSellableEnhanced(item)
	if not item then return false end

	print("🔍 ShopSystem: Enhanced sellable check for: " .. (item.id or "unknown"))

	-- EXPLICIT CROP CHECK - highest priority
	if item.category == "crops" or item.type == "crop" then
		print("🌾 CROP DETECTED: " .. item.id)

		-- Check if explicitly marked as not sellable
		if item.notSellable then
			print("❌ Crop marked as not sellable: " .. item.id)
			return false
		end

		-- Default crops to sellable unless explicitly false
		if item.sellable ~= false then
			print("✅ CROP IS SELLABLE: " .. item.id)
			return true
		end
	end
	...
	for _, cropId in ipairs(knownCrops) do
		if item.id == cropId then
			print("🌱 KNOWN CROP ID MATCH: " .. item.id)
			return item.sellable ~= false and not item.notSellable
		end
	end

	-- MILK AND LIVESTOCK PRODUCTS
	if item.id == "milk" or item.id == "fresh_milk" then
		print("🥛 MILK PRODUCT DETECTED: " .. item.id)
		return true
	end

	-- EXPLICIT SELLABLE FLAG
	if item.sellable == true then
		print("✅ EXPLICITLY MARKED SELLABLE: " .. item.id)
		return true
	end

	-- SELLABLE TYPES AND CATEGORIES
	local sellableTypes = {"crop", "material", "livestock_product"}
	local sellableCategories = {"crops", "materials", "livestock"}

	if item.type then
		for _, sellableType in ipairs(sellableTypes) do
			if item.type == sellableType then
	...
	print("❌ ITEM NOT SELLABLE: " .. item.id)
	return false
end

-- FIXED: Enhanced GetPlayerStock with better crop detection
function ShopSystem:GetPlayerStockEnhanced(playerData, itemId)
	print("🔍 ShopSystem: Enhanced stock search for: " .. itemId)
	...
	-- PRIORITY 1: Check farming inventory (where crops are stored)
	if playerData.farming and playerData.farming.inventory then
		local farmingStock = playerData.farming.inventory[itemId] or 0
		if farmingStock > 0 then
			totalStock = totalStock + farmingStock
			print("🌾 Found " .. farmingStock .. "x " .. itemId .. " in farming inventory")
		end
	end

	-- PRIORITY 2: Milk special handling
	if itemId == "milk" or itemId == "fresh_milk" then
		-- Check direct milk property
		...
		if playerData.livestock and playerData.livestock.inventory and playerData.livestock.inventory.milk then
			local livestockMilk = playerData.livestock.inventory.milk
			totalStock = totalStock + livestockMilk
			print("🥛 Found " .. livestockMilk .. " milk in livestock inventory")
		end
	end

	-- PRIORITY 3: Check other inventories
	...
	for _, pathArray in ipairs(otherInventories) do
		local inventory = playerData
		...
		if pathValid and inventory and inventory[itemId] then
			...
			if type(amount) == "number" and amount > 0 then
				...
				print("📦 Found " .. amount .. "x " .. itemId .. " in " .. table.concat(pathArray, "."))
			end
		end
	end
...
-- FIXED: Enhanced RemovePlayerItems with better crop handling
function ShopSystem:RemovePlayerItemsEnhanced(playerData, itemId, quantity)
	print("🗑️ ShopSystem: Enhanced removal of " .. quantity .. "x " .. itemId)

	if not playerData then
		warn("No player data provided for item removal")
		return false
	end

	local remainingToRemove = quantity

	-- SPECIAL HANDLING FOR CROPS - Check farming inventory FIRST
	if self:IsCropItem(itemId) then
		print("🌾 Detected crop item, checking farming inventory first")
		...
		if playerData.farming and playerData.farming.inventory and playerData.farming.inventory[itemId] then
			...
			local removeFromFarming = math.min(farmingAmount, remainingToRemove)

			playerData.farming.inventory[itemId] = farmingAmount - removeFromFarming
			remainingToRemove = remainingToRemove - removeFromFarming

			print("🌾 Removed " .. removeFromFarming .. "x " .. itemId .. " from farming inventory")
			...
			-- If we got everything from farming inventory, we're done
			if remainingToRemove <= 0 then
				print("✅ Successfully removed all " .. quantity .. "x " .. itemId .. " from farming inventory")
				return true
			end
		end
	end
	...
	if itemId == "milk" or itemId == "fresh_milk" then
		print("🥛 Special milk removal handling")

		-- Remove from direct milk property
		...
		if remainingToRemove > 0 and playerData.livestock and playerData.livestock.inventory and playerData.livestock.inventory.milk then
			local livestockMilk = playerData.livestock.inventory.milk
			local removeFromLivestock = math.min(livestockMilk, remainingToRemove)

			playerData.livestock.inventory.milk = livestockMilk - removeFromLivestock
			remainingToRemove = remainingToRemove - removeFromLivestock
			print("🥛 Removed " .. removeFromLivestock .. " milk from livestock inventory")
	...
	-- GENERAL INVENTORY REMOVAL for remaining items
	if remainingToRemove > 0 then
		local inventoryPaths = {
			{"livestock", "inventory"},
			{"inventory"}, 
		...
		for _, pathArray in ipairs(inventoryPaths) do
			...
			if pathValid and inventory and inventory[itemId] then
				...
				if type(availableAmount) == "number" and availableAmount > 0 then
					...
					print("📦 Removed " .. removeAmount .. "x " .. itemId .. " from " .. table.concat(pathArray, "."))
...
-- Helper function to detect crop items
function ShopSystem:IsCropItem(itemId)
	local knownCrops = {
		"carrot", "corn", "strawberry", "wheat", "potato", 
		"tomato", "cabbage", "radish", "broccoli", "golden_fruit", "glorious_sunflower"
	}
	...
	-- Check if item has crop-like properties
	local item = self:GetShopItemByIdEnhanced(itemId)
	if item then
		return item.category == "crops" or item.type == "crop"
	end

	return false
end

-- ENHANCED HandleSell method using the new functions
function ShopSystem:HandleSellEnhanced(player, itemId, quantity)
	print("🏪 ShopSystem: ENHANCED HandleSell - " .. player.Name .. " selling " .. quantity .. "x " .. itemId)
	...
	-- Check if player has the item using enhanced method
	local playerStock = self:GetPlayerStockEnhanced(playerData, itemId)
	...
	-- Get item data using enhanced method
	local item = self:GetShopItemByIdEnhanced(itemId)
	if not item then
	...
	-- Check if item is sellable using enhanced method
	if not self:IsItemSellableEnhanced(item) then
		self:SendNotification(player, "Cannot Sell", "This item cannot be sold!", "error")
		return false
	end

	-- Calculate sell price with enhanced logic
	local sellPrice = self:CalculateSellPrice(item)
	if sellPrice <= 0 then
	...
	-- Remove items using enhanced method
	local removeSuccess = self:RemovePlayerItemsEnhanced(playerData, itemId, quantity)
...
function ShopSystem:CalculateSellPrice(item)
	-- Check for explicit sell value first
	if item.sellValue and item.sellValue > 0 then
		return item.sellValue
	end

	-- Use ItemConfig function if available
	if self.ItemConfig and self.ItemConfig.GetCropSellPriceEnhanced then
		local cropPrice = self.ItemConfig.GetCropSellPriceEnhanced(item.id)
		if cropPrice and cropPrice > 0 then
			return cropPrice
		end
	end

	-- Fallback to 50% of purchase price
...
function ShopSystem:DebugCropSelling(player)
	print("=== ENHANCED CROP SELLING DEBUG ===")
	...
	print("\n🧪 Testing crop selling system:")
	for _, cropId in ipairs(testCrops) do
		print("\n--- Testing: " .. cropId .. " ---")

		-- Check if item exists
		local item = self:GetShopItemByIdEnhanced(cropId)
		print("Item found: " .. tostring(item ~= nil))

		if item then
			print("Category: " .. (item.category or "N/A"))
			print("Type: " .. (item.type or "N/A"))
			print("Sellable: " .. tostring(self:IsItemSellableEnhanced(item)))
			print("Sell price: " .. self:CalculateSellPrice(item))
		end

		-- Check player stock
		local stock = self:GetPlayerStockEnhanced(playerData, cropId)
		print("Player stock: " .. stock)
	end

	print("\n📦 Inventory contents:")
	if playerData.farming and playerData.farming.inventory then
		print("Farming inventory:")
		for itemId, amount in pairs(playerData.farming.inventory) do
			print("  " .. itemId .. ": " .. amount)
		end
	else
...
game:GetService("Players").PlayerAdded:Connect(function(player)
	player.Chatted:Connect(function(message)
		if player.Name == "TommySalami311" then -- Replace with your username
			...
			if command == "/debugcropsell" then
				_G.ShopSystem:DebugCropSelling(player)

			elseif command == "/testcropsell" then
				local cropId = args[2] or "carrot"
				local amount = tonumber(args[3]) or 1
				print("Testing sale of " .. amount .. "x " .. cropId)
				_G.ShopSystem:HandleSellEnhanced(player, cropId, amount)
...
print("ShopSystem: ✅ Enhanced crop selling system loaded!")
print("🔧 Debug Commands:")
print("  /debugcropsell - Comprehensive crop selling debug")
print("  /testcropsell [crop] [amount] - Test selling specific crop")
