...
function ShopSystem:ValidateItemEnhanced(item, itemId)
	...
	if not isValidCurrency then
		warn("ShopSystem: Item " .. itemId .. " has invalid currency: " .. tostring(item.currency) .. ". Valid currencies: " .. table.concat(validCurrencies, ", "))
		return false
	end

	-- Validate Robux items have product IDs
	if item.currency == "Robux" then
		if not self.RobuxProducts[itemId] then
			warn("ShopSystem: Robux item " .. itemId .. " missing product ID")
			-- Don't fail validation, just warn for now
		end
	end

	-- Validate cow-specific properties with better error messages
...
function ShopSystem:ValidateCowPurchase(player, playerData, item, quantity)
	print("🐄 ShopSystem: Validating cow purchase - " .. item.id)
	...
	-- For basic cow (first cow), be more lenient
	if item.id == "basic_cow" then
		print("🐄 ShopSystem: Basic cow purchase - allowing with minimal requirements")
	...
	-- Check cow capacity for non-basic cows
	...
	elseif item.type == "cow_upgrade" then
		...
		if #eligibleCows < quantity then
			...
			return false, "You need " .. quantity .. "x " .. upgradeFrom .. " cows to upgrade!"
		end
	end

	print("🐄 ShopSystem: Cow purchase validation passed")
...
function ShopSystem:RemovePlayerItems(playerData, itemId, quantity)
	print("ShopSystem: Removing " .. quantity .. "x " .. itemId .. " from player inventory")
	...
	-- Find the item in player inventories
	for _, path in ipairs(inventoryPaths) do
		...
		-- Check if item exists in this inventory
		if inventory and inventory[itemId] and inventory[itemId] >= quantity then
			...
			print("ShopSystem: Removed " .. quantity .. "x " .. itemId .. " from " .. table.concat(path, "."))
			return true
		end
	end

	warn("ShopSystem: Could not find " .. quantity .. "x " .. itemId .. " in any player inventory")
	return false
end
...
function ShopSystem:IsItemSellable(item)
	...
	-- Special cases
	if item.id == "milk" or item.id:find("crop") or item.id:find("seed") then
		return true
	end
