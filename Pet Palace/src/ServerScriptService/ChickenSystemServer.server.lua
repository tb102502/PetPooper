...
function ChickenSystem:CreateBasicChickenModel(chickenInstance)
	print("ChickenSystem: Creating basic fallback chicken model for " .. chickenInstance.chickenType)

	local chickenType = chickenInstance.chickenType
	local position = chickenInstance.currentPosition

	-- Create a basic chicken model from scratch
	...
	body.BrickColor = self:GetChickenColor(chickenType)
	body.Anchored = true
	...
	head.BrickColor = self:GetChickenColor(chickenType)
	head.Anchored = true
	head.CanCollide = false
	head.CFrame = CFrame.new(position + Vector3.new(0, 1, 1.5))
	head.Parent = chickenModel

	-- Create beak
	local beak = Instance.new("Part")
	beak.Name = "Beak"
	beak.Size = Vector3.new(0.3, 0.2, 0.5)
	...
	beak.CFrame = CFrame.new(position + Vector3.new(0, 1, 2))
	beak.Parent = chickenModel

	-- Create legs
	for i = 1, 2 do
		local leg = Instance.new("Part")
		leg.Name = (i == 1) and "Left Leg" or "Right Leg"
		leg.Size = Vector3.new(0.3, 1, 0.3)
		leg.Shape = Enum.PartType.Cylinder
		leg.Material = Enum.Material.SmoothPlastic
		leg.BrickColor = BrickColor.new("Bright yellow")
		leg.Anchored = true
		leg.CanCollide = false
		local xOffset = (i == 1) and -0.5 or 0.5
		leg.CFrame = CFrame.new(position + Vector3.new(xOffset, -1.25, 0))
		leg.Parent = chickenModel
	end

	-- Create tail feathers
	local tail = Instance.new("Part")
	tail.Name = "Tail"
	tail.Size = Vector3.new(1, 2, 0.5)
	tail.Shape = Enum.PartType.Block
	tail.Material = Enum.Material.SmoothPlastic
	tail.BrickColor = self:GetChickenColor(chickenType)
	tail.Anchored = true
	tail.CanCollide = false
	tail.CFrame = CFrame.new(position + Vector3.new(0, 0.5, -2))
	tail.Parent = chickenModel

	-- Add special effects for different chicken types
	if chickenType == "guinea_fowl" then
		-- Add spots
		for i = 1, 5 do
			local spot = Instance.new("SurfaceGui")
			spot.Face = Enum.NormalId.Top
			spot.Parent = body

			local spotFrame = Instance.new("Frame")
			spotFrame.Size = UDim2.new(0.2, 0, 0.2, 0)
			spotFrame.Position = UDim2.new(math.random() * 0.8, 0, math.random() * 0.8, 0)
			spotFrame.BackgroundColor3 = Color3.new(0, 0, 0)
			spotFrame.BorderSizePixel = 0
			spotFrame.Parent = spot

			local corner = Instance.new("UICorner")
			corner.CornerRadius = UDim.new(0.5, 0)
			corner.Parent = spotFrame
		end
	elseif chickenType == "rooster" then
		-- Add comb (red thing on head)
		local comb = Instance.new("Part")
		comb.Name = "Comb"
		comb.Size = Vector3.new(0.8, 0.5, 0.3)
		comb.Shape = Enum.PartType.Block
		comb.Material = Enum.Material.SmoothPlastic
		comb.BrickColor = BrickColor.new("Bright red")
		comb.Anchored = true
		comb.CanCollide = false
		comb.CFrame = CFrame.new(position + Vector3.new(0, 1.5, 1.2))
		comb.Parent = chickenModel

		-- Make body slightly larger
		body.Size = Vector3.new(2.5, 2, 3.5)
	end

	print("ChickenSystem: Created basic chicken model successfully")
	return chickenModel
end

function ChickenSystem:GetChickenColor(chickenType)
	local colors = {
		basic_chicken = BrickColor.new("White"),
		guinea_fowl = BrickColor.new("Light grey"),
		rooster = BrickColor.new("Reddish brown")
	}
	return colors[chickenType] or BrickColor.new("White")
end

-- ========== ENHANCED DEBUGGING ==========

function ChickenSystem:DebugChickenSpawn(player, chickenType)
	...
	if spawnPos then
		print("Attempting to spawn " .. chickenType .. " at position " .. tostring(spawnPos))
		local result = self:SpawnChicken(player, chickenType, spawnPos)
		print("Spawn result:", result ~= nil and "SUCCESS" or "FAILED")
	else
		print("❌ Could not find spawn position!")
	end

	-- Check system state
	print("ChickenSystem state:")
	print("  ActiveChickens count:", self:CountTable(self.ActiveChickens))
	print("  ChickenModels count:", self:CountTable(self.ChickenModels))

	if self.ActiveChickens[player.UserId] then
		print("  Player chickens:")
		for id, chicken in pairs(self.ActiveChickens[player.UserId]) do
			print("    " .. id .. ": " .. chicken.chickenType)
		end
	end
...
function ChickenSystem:CountTable(t)
	local count = 0
...
-- ========== ENHANCED SPAWN POSITION DETECTION ==========

function ChickenSystem:FindChickenSpawnPositionEnhanced(player)
	print("ChickenSystem: Enhanced spawn position finding for " .. player.Name)

	local spawnPosition = nil
	local method = "unknown"

	-- Method 1: Try GameCore farm plot position
	if _G.GameCore and _G.GameCore.GetFarmPlotPosition then
		local success, plotCFrame = pcall(function()
			return _G.GameCore:GetFarmPlotPosition(player, 1)
		end)

		if success and plotCFrame then
			spawnPosition = plotCFrame.Position + Vector3.new(10, 3, 10)
			method = "GameCore.GetFarmPlotPosition"
			print("✅ Method 1 SUCCESS: " .. method .. " -> " .. tostring(spawnPosition))
		else
			print("❌ Method 1 FAILED: GameCore.GetFarmPlotPosition error: " .. tostring(plotCFrame))
		end
	else
		print("❌ Method 1 FAILED: GameCore or GetFarmPlotPosition not available")
	end

	-- Method 2: Manual workspace search
	if not spawnPosition then
		...
		if areas then
			...
			if starterMeadow then
				...
				if farmArea then
					...
					print("Player farm '" .. player.Name .. "_Farm' exists:", playerFarm ~= nil)
					...
					if playerFarm then
						...
						for _, child in pairs(playerFarm:GetChildren()) do
							...
							if child:IsA("Model") and child.Name:find("FarmPlot") then
								local primaryPart = child.PrimaryPart or child:FindFirstChild("BasePart")
								if primaryPart then
									spawnPosition = primaryPart.Position + Vector3.new(10, 3, 10)
									method = "Manual workspace search"
									print("✅ Method 2 SUCCESS: " .. method .. " -> " .. tostring(spawnPosition))
									break
								end
							end
						end
		...
		if not spawnPosition then
			print("❌ Method 2 FAILED: Manual workspace search")
		end
	end

	-- Method 3: Near player character
	if not spawnPosition and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
		spawnPosition = player.Character.HumanoidRootPart.Position + Vector3.new(5, 2, 5)
		method = "Near player character"
		print("✅ Method 3 SUCCESS: " .. method .. " -> " .. tostring(spawnPosition))
	end

	-- Method 4: World fallback
	if not spawnPosition then
		spawnPosition = Vector3.new(0, 10, 0)
		method = "World fallback"
		print("⚠️ Method 4 FALLBACK: " .. method .. " -> " .. tostring(spawnPosition))
	end

	print("Final spawn position: " .. tostring(spawnPosition) .. " (via " .. method .. ")")
	return spawnPosition
end

-- Replace the original FindChickenSpawnPosition with the enhanced version
ChickenSystem.FindChickenSpawnPosition = ChickenSystem.FindChickenSpawnPositionEnhanced

-- ========== SHOP INTEGRATION FIX ==========

-- Add this method to handle purchases from shop
function ChickenSystem:HandlePurchaseFromShop(player, itemId)
	print("ChickenSystem: Handling purchase of " .. itemId .. " for " .. player.Name)

	-- Map item IDs to chicken types
	local chickenMap = {
		basic_chicken = "basic_chicken",
		guinea_fowl = "guinea_fowl", 
		rooster = "rooster"
	}

	local chickenType = chickenMap[itemId]
	if not chickenType then
		warn("ChickenSystem: Unknown chicken item: " .. itemId)
		return false
	end

	return self:HandleChickenPurchase(player, chickenType)
end
...
Players.PlayerAdded:Connect(function(player)
	player.Chatted:Connect(function(message)
		if player.Name == "TommySalami311" then -- Replace with your username
			...
			if command == "/testchicken" then
				local chickenType = args[2] or "basic_chicken"
				ChickenSystem:DebugChickenSpawn(player, chickenType)

			elseif command == "/forcespawnchicken" then
				local chickenType = args[2] or "basic_chicken"
				local position = player.Character and player.Character.HumanoidRootPart.Position + Vector3.new(5, 2, 0) or Vector3.new(0, 5, 0)
				local result = ChickenSystem:SpawnChicken(player, chickenType, position)
				print("Force spawn result:", result ~= nil and "SUCCESS" or "FAILED")

			elseif command == "/listchickens" then
				print("=== ACTIVE CHICKENS ===")
				for userId, chickens in pairs(ChickenSystem.ActiveChickens) do
					local player = Players:GetPlayerByUserId(userId)
					local playerName = player and player.Name or "Unknown"
					print("Player: " .. playerName .. " (ID: " .. userId .. ")")
					for chickenId, chicken in pairs(chickens) do
						print("  " .. chickenId .. ": " .. chicken.chickenType .. " at " .. tostring(chicken.currentPosition))
					end
				end

			elseif command == "/clearallchickens" then
				-- Remove all chickens
				for userId, chickens in pairs(ChickenSystem.ActiveChickens) do
					for chickenId, chicken in pairs(chickens) do
						ChickenSystem:RemoveChicken(chicken)
					end
				end
				ChickenSystem.ActiveChickens = {}
				ChickenSystem.ChickenModels = {}
				print("All chickens cleared")

			elseif command == "/debugfarmpos" then
				if _G.GameCore and _G.GameCore.GetFarmPlotPosition then
					local success, pos = pcall(function()
						return _G.GameCore:GetFarmPlotPosition(player, 1)
					end)
					print("Farm plot position result:", success, pos)
				else
					print("GameCore.GetFarmPlotPosition not available")
				end
			end
		end
	end)
...
print("ChickenSystem: ✅ FIXES APPLIED!")
print("🔧 New Features:")
print("  🐔 Fallback chicken model creation")
print("  🔍 Enhanced spawn position detection")
print("  📊 Detailed debugging")
print("  🛒 Shop integration handling")
print("")
print("🧪 Admin Commands:")
print("  /testchicken [type] - Debug spawn process")
print("  /forcespawnchicken [type] - Force spawn near player")
print("  /listchickens - Show all active chickens")
print("  /clearallchickens - Remove all chickens")
print("  /debugfarmpos - Test farm position detection")
...
-- CHICKEN SPAWNING PROCESS DEBUGGER
-- Add this to your ChickenSystemServer.server.lua to trace exactly what's happening

-- ========== COMPREHENSIVE SPAWN DEBUGGING ==========

function ChickenSystem:DebugEntireSpawnProcess(player, chickenType)
	print("🔍 === COMPLETE CHICKEN SPAWN DEBUG FOR " .. player.Name .. " ===")
	print("Chicken Type: " .. chickenType)
	print("Timestamp: " .. os.time())

	local step = 1
	local function logStep(message, success)
		local status = success and "✅" or "❌"
		print("Step " .. step .. ": " .. status .. " " .. message)
		step = step + 1
		return success
	end

	-- Step 1: Check if ChickenSystem is initialized
	if not logStep("ChickenSystem initialized", self.ActiveChickens ~= nil) then
		print("❌ CRITICAL: ChickenSystem not properly initialized!")
		return false
	end

	-- Step 2: Check spawn position
	local spawnPosition = self:FindChickenSpawnPosition(player)
	if not logStep("Spawn position found: " .. tostring(spawnPosition), spawnPosition ~= nil) then
		print("❌ CRITICAL: Could not find spawn position!")
		return false
	end

	-- Step 3: Try to spawn the chicken
	print("🐔 Attempting to spawn chicken...")
	local chickenInstance = self:SpawnChicken(player, chickenType, spawnPosition)
	if not logStep("Chicken instance created", chickenInstance ~= nil) then
		print("❌ CRITICAL: SpawnChicken returned nil!")
		return false
	end

	-- Step 4: Check if chicken was stored in ActiveChickens
	local userId = player.UserId
	local storedInActive = self.ActiveChickens[userId] and self.ActiveChickens[userId][chickenInstance.chickenId] ~= nil
	if not logStep("Chicken stored in ActiveChickens", storedInActive) then
		print("❌ WARNING: Chicken not found in ActiveChickens!")
	end

	-- Step 5: Check if chicken model was created
	local modelExists = self.ChickenModels[chickenInstance.chickenId] ~= nil
	if not logStep("Chicken model created", modelExists) then
		print("❌ WARNING: Chicken model not found in ChickenModels!")
	end

	-- Step 6: Check if model is in workspace
	local modelInWorkspace = false
	if self.ChickenModels[chickenInstance.chickenId] then
		modelInWorkspace = self.ChickenModels[chickenInstance.chickenId].Parent == workspace
	end
	if not logStep("Chicken model in workspace", modelInWorkspace) then
		print("❌ WARNING: Chicken model not in workspace!")
	end

	-- Step 7: Check model position
	if self.ChickenModels[chickenInstance.chickenId] then
		local model = self.ChickenModels[chickenInstance.chickenId]
		local modelPos = model.PrimaryPart and model.PrimaryPart.Position or "No PrimaryPart"
		logStep("Model position: " .. tostring(modelPos), true)

		-- Check if model is visible (not underground)
		if model.PrimaryPart then
			local pos = model.PrimaryPart.Position
			local isUnderground = pos.Y < 0
			logStep("Model above ground (Y=" .. pos.Y .. ")", not isUnderground)
		end
	end

	-- Step 8: Count total chickens
	local totalChickens = 0
	for userId, chickens in pairs(self.ActiveChickens) do
		for _, _ in pairs(chickens) do
			totalChickens = totalChickens + 1
		end
	end
	logStep("Total active chickens: " .. totalChickens, true)

	-- Step 9: Check workspace for ALL chicken models
	local workspaceChickens = {}
	for _, obj in pairs(workspace:GetChildren()) do
		if obj.Name:find("Chicken_") or obj.Name:find("chicken") then
			table.insert(workspaceChickens, obj.Name .. " at " .. tostring(obj:FindFirstChild("PrimaryPart") and obj.PrimaryPart.Position or "No position"))
		end
	end
	print("🌍 Chickens found in workspace:")
	if #workspaceChickens == 0 then
		print("  ❌ No chicken models found in workspace!")
	else
		for _, chicken in ipairs(workspaceChickens) do
			print("  🐔 " .. chicken)
		end
	end
	...
	if chickenInstance then
		print("✅ Spawn process completed with chicken ID: " .. chickenInstance.chickenId)
		print("📍 Expected position: " .. tostring(spawnPosition))
		return chickenInstance
	else
		print("❌ Spawn process failed!")
		return nil
	end
end

-- ========== ENHANCED SPAWN CHICKEN WITH DEBUGGING ==========

-- Replace or enhance your existing SpawnChicken method
function ChickenSystem:SpawnChickenWithDebug(player, chickenType, position)
	print("🐔 SpawnChickenWithDebug called:")
	print("  Player: " .. player.Name)
	print("  Type: " .. chickenType)
	print("  Position: " .. tostring(position))

	-- Check chicken configuration
	local chickenData = CHICKEN_CONFIG.chickenTypes[chickenType]
	if not chickenData then
		warn("❌ Invalid chicken type: " .. chickenType)
		return nil
	end
	print("✅ Chicken config found: " .. chickenData.name)

	-- Generate unique ID
	local chickenId = self:GenerateChickenId()
	print("✅ Generated chicken ID: " .. chickenId)

	-- Create chicken instance data
	local chickenInstance = {
		chickenType = chickenType,
		player = player,
		spawnTime = os.time(),
	...
	print("✅ Chicken instance data created")

	-- Store in active chickens
	if not self.ActiveChickens[player.UserId] then
		self.ActiveChickens[player.UserId] = {}
		print("✅ Created ActiveChickens entry for player")
	end
	self.ActiveChickens[player.UserId][chickenId] = chickenInstance
	print("✅ Stored chicken in ActiveChickens")

	-- Create visual model
	print("🎨 Creating chicken model...")
	local chickenModel = self:CreateChickenModelFromTemplate(chickenInstance)
	if chickenModel then
		self.ChickenModels[chickenId] = chickenModel
		print("✅ Chicken model created and stored")

		-- Verify model is in workspace
		if chickenModel.Parent == workspace then
			print("✅ Model confirmed in workspace")
		else
			warn("❌ Model not in workspace! Parent: " .. tostring(chickenModel.Parent))
		end

		-- Check model position
		if chickenModel.PrimaryPart then
			print("✅ Model position: " .. tostring(chickenModel.PrimaryPart.Position))
		else
			warn("❌ Model has no PrimaryPart!")
		end
	else
		warn("❌ Failed to create chicken model!")
		-- Clean up on model creation failure
		self.ActiveChickens[player.UserId][chickenId] = nil
		return nil
	end

	-- Setup patrol route
	self:SetupChickenPatrol(chickenInstance)
	print("✅ Patrol route setup complete")
	...
	if GameCore and GameCore.SendNotification then
		GameCore:SendNotification(player, "🐔 Chicken Deployed!", 
			chickenData.name .. " is now protecting your farm!", "success")
		print("✅ Notification sent")
	end

	print("🎉 Chicken spawn completed successfully!")
	return chickenInstance
end

-- ========== MODEL CREATION DEBUGGING ==========

function ChickenSystem:CreateChickenModelFromTemplateWithDebug(chickenInstance)
	print("🎨 CreateChickenModelFromTemplate called for " .. chickenInstance.chickenType)

	local chickenType = chickenInstance.chickenType
	local position = chickenInstance.currentPosition

	print("📍 Target position: " .. tostring(position))

	-- Try to find chicken model templates
	local replicatedStorage = game:GetService("ReplicatedStorage")
	local chickenModels = replicatedStorage:FindFirstChild("ChickenModels")

	if not chickenModels then
		print("⚠️ ChickenModels folder not found, using fallback creation")
		return self:CreateBasicChickenModel(chickenInstance)
	end
	...
	local template = chickenModels:FindFirstChild(templateName)
	if not template then
		print("⚠️ Template " .. templateName .. " not found, using fallback")
		return self:CreateBasicChickenModel(chickenInstance)
	end
	...
	local chickenModel = template:Clone()
	chickenModel.Name = "Chicken_" .. chickenInstance.chickenId
	print("✅ Cloned template with name: " .. chickenModel.Name)

	-- Position the model
	if chickenModel.PrimaryPart then
		chickenModel:SetPrimaryPartCFrame(CFrame.new(position))
		print("✅ Positioned using PrimaryPart")
	else
		print("⚠️ No PrimaryPart found, trying alternative positioning")
		for _, part in pairs(chickenModel:GetChildren()) do
			if part:IsA("BasePart") then
				part.Position = position
				print("✅ Positioned using part: " .. part.Name)
				break
			end
		end
	end
	...
	-- Add attributes
	chickenModel:SetAttribute("CowId", chickenInstance.chickenId)
	chickenModel:SetAttribute("Owner", chickenInstance.player.Name)
	chickenModel:SetAttribute("Tier", chickenInstance.chickenType)
	print("✅ Attributes set")

	-- Create health bar and nameplate
	local healthBar = self:CreateChickenHealthBar(chickenInstance)
	local nameplate = self:CreateChickenNameplate(chickenInstance)
	print("✅ Health bar and nameplate created")

	print("🎉 Chicken model creation complete!")
	return chickenModel
end

-- ========== WORKSPACE SCANNING ==========

function ChickenSystem:ScanWorkspaceForChickens(player)
	print("🔍 Scanning workspace for chickens...")

	local foundChickens = {}
	local playerChickens = {}

	for _, obj in pairs(workspace:GetChildren()) do
		-- Look for anything that might be a chicken
		if obj:IsA("Model") and (obj.Name:find("Chicken") or obj.Name:find("chicken")) then
			local owner = obj:GetAttribute("Owner")
			local chickenId = obj:GetAttribute("CowId") -- Note: might be CowId due to copy-paste
			local tier = obj:GetAttribute("Tier")

			local info = {
				name = obj.Name,
				owner = owner or "Unknown",
				chickenId = chickenId or "No ID",
				tier = tier or "Unknown",
				position = obj.PrimaryPart and obj.PrimaryPart.Position or "No PrimaryPart",
				parentName = obj.Parent and obj.Parent.Name or "No Parent"
			}

			table.insert(foundChickens, info)

			if owner == player.Name then
				table.insert(playerChickens, info)
			end
		end
	end

	print("📊 Workspace scan results:")
	print("  Total chicken models found: " .. #foundChickens)
	print("  Your chicken models: " .. #playerChickens)

	if #foundChickens == 0 then
		print("  ❌ No chicken models found in workspace!")
	else
		print("  🐔 All chickens found:")
		for i, chicken in ipairs(foundChickens) do
			print("    " .. i .. ". " .. chicken.name .. " (Owner: " .. chicken.owner .. ", Pos: " .. tostring(chicken.position) .. ")")
		end
	end

	if #playerChickens == 0 then
		print("  ❌ No chickens found for " .. player.Name)
	else
		print("  🎯 Your chickens:")
		for i, chicken in ipairs(playerChickens) do
			print("    " .. i .. ". " .. chicken.name .. " at " .. tostring(chicken.position))
		end
	end

	return foundChickens, playerChickens
end
...
Players.PlayerAdded:Connect(function(player)
	player.Chatted:Connect(function(message)
		if player.Name == "TommySalami311" then -- Replace with your username
			...
			if command == "/debugspawn" then
				local chickenType = args[2] or "basic_chicken"
				ChickenSystem:DebugEntireSpawnProcess(player, chickenType)

			elseif command == "/testspawnwith" then
				local chickenType = args[2] or "basic_chicken"
				local position = player.Character and player.Character.HumanoidRootPart.Position + Vector3.new(5, 2, 0) or Vector3.new(0, 5, 0)
				local result = ChickenSystem:SpawnChickenWithDebug(player, chickenType, position)
				print("Spawn with debug result:", result and "SUCCESS" or "FAILED")

			elseif command == "/scanworkspace" then
				ChickenSystem:ScanWorkspaceForChickens(player)

			elseif command == "/checkchickensystem" then
				print("=== CHICKEN SYSTEM STATUS ===")
				print("ActiveChickens initialized:", ChickenSystem.ActiveChickens ~= nil)
				print("ChickenModels initialized:", ChickenSystem.ChickenModels ~= nil)
				print("Player entries in ActiveChickens:")
				for userId, chickens in pairs(ChickenSystem.ActiveChickens or {}) do
					local player = Players:GetPlayerByUserId(userId)
					local playerName = player and player.Name or "Unknown"
					local count = 0
					for _ in pairs(chickens) do count = count + 1 end
					print("  " .. playerName .. ": " .. count .. " chickens")
				end

			elseif command == "/teleporttochicken" then
				local _, playerChickens = ChickenSystem:ScanWorkspaceForChickens(player)
				if #playerChickens > 0 and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
					local chicken = playerChickens[1]
					if type(chicken.position) == "userdata" then -- Vector3
						player.Character.HumanoidRootPart.Position = chicken.position + Vector3.new(0, 5, 0)
						print("🚀 Teleported to your chicken: " .. chicken.name)
					end
				else
					print("❌ No chickens found to teleport to")
				end
			end
		end
	end)
...
print("🔍 COMPREHENSIVE CHICKEN SPAWN DEBUGGER LOADED!")
print("")
print("🎮 Debug Commands:")
print("  /debugspawn [type] - Complete spawn process debug")
print("  /testspawnwith [type] - Test spawn with full debugging")
print("  /scanworkspace - Find all chicken models in workspace")
print("  /checkchickensystem - Check ChickenSystem status")
print("  /teleporttochicken - Teleport to your first chicken")
