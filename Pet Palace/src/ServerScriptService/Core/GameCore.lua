...
function GameCore:CreatePlayerFarmPlot(player, plotNumber)
	...
	-- Create soil base (larger to accommodate multiple planting spots)
	local soil = Instance.new("Part")
	soil.Name = "Soil"
	soil.Size = Vector3.new(12, 1, 12) -- Increased size for 3x3 grid
	...
	border.Name = "Border"
	border.Size = Vector3.new(12.5, 0.2, 12.5)
	...
	-- Create 3x3 grid of planting spots (9 total spots)
	local spotSize = 3 -- Size of each planting spot
	local spacing = 4 -- Distance between spot centers
	local startOffset = -4 -- Starting position offset

	for row = 1, 3 do
		for col = 1, 3 do
			local spotNumber = (row - 1) * 3 + col
			local spotPosition = soil.Position + Vector3.new(
				startOffset + (col - 1) * spacing,
				0.6, -- Slightly above soil
				startOffset + (row - 1) * spacing
			)

			local plantingSpot = self:CreatePlantingSpot(spotNumber, spotPosition, spotSize)
			plantingSpot.Parent = plantingSpotsFolder
		end
	end
	...
	sign.Position = soil.Position + Vector3.new(6.5, 1.5, 6.5)
	sign.Anchored = true
	...
	plotModel:SetAttribute("MaxSpots", 9)
	plotModel:SetAttribute("UsedSpots", 0)

	-- Initialize spot data
	for spotId = 1, 9 do
		plotModel:SetAttribute("Spot" .. spotId .. "_IsPlanted", false)
		plotModel:SetAttribute("Spot" .. spotId .. "_PlantType", "")
		plotModel:SetAttribute("Spot" .. spotId .. "_GrowthStage", 0)
		plotModel:SetAttribute("Spot" .. spotId .. "_PlantTime", 0)
		plotModel:SetAttribute("Spot" .. spotId .. "_TimeToGrow", 0)
	end
	...
	print("GameCore: Created farm plot " .. plotNumber .. " with 9 planting spots for " .. player.Name)
	return true
end

-- Create individual planting spot
function GameCore:CreatePlantingSpot(spotNumber, position, size)
	local spotModel = Instance.new("Model")
	spotModel.Name = "PlantingSpot_" .. spotNumber

	-- Main planting area (invisible base)
	local plantingArea = Instance.new("Part")
	plantingArea.Name = "PlantingArea"
	plantingArea.Size = Vector3.new(size, 0.1, size)
	plantingArea.Position = position
	plantingArea.Anchored = true
	plantingArea.CanCollide = false
	plantingArea.Transparency = 1 -- Invisible base
	plantingArea.Parent = spotModel

	-- Glowing border rectangle
	local glowBorder = Instance.new("Part")
	glowBorder.Name = "GlowBorder"
	glowBorder.Size = Vector3.new(size + 0.2, 0.1, size + 0.2)
	glowBorder.Position = position + Vector3.new(0, 0.05, 0)
	...
	glowBorder.Color = Color3.fromRGB(100, 255, 100) -- Green for empty
	glowBorder.Transparency = 0.7
	glowBorder.Parent = spotModel

	-- Corner indicators (small glowing cubes at corners)
	for i = 1, 4 do
		...
		corner.Color = Color3.fromRGB(100, 255, 100)
		corner.Transparency = 0.5
		corner.Shape = Enum.PartType.Block
		corner.Parent = spotModel

		-- Position corners
		local cornerOffset = size / 2 - 0.1
		if i == 1 then -- Top-left
			corner.Position = position + Vector3.new(-cornerOffset, 0.2, -cornerOffset)
	...
	-- Click detector for individual spot
	local clickDetector = Instance.new("ClickDetector")
	clickDetector.MaxActivationDistance = 20
	clickDetector.Parent = plantingArea

	-- Store spot attributes
	...
	spotModel.PrimaryPart = plantingArea

	print("GameCore: Created planting spot " .. spotNumber .. " at " .. tostring(position))
	return spotModel
end
...
function GameCore:CreatePetModel(petConfig, position)
	...
	-- FIXED: Declare rootPart as local
	...
	if not rootPart then
		...
		if rootPart then
			...
			-- FIXED: Declare targetPart as local within scope
			local targetPart = nil
...
function GameCore:StartPetBehavior(petModel, petConfig)
	...
	local glowEffect = nil -- Also declared as local
	...
	-- FIXED: Declare behaviorConnection as local
