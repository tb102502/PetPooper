--[[
    GameCore.lua - COMPLETE UPDATED GAME CORE SYSTEM
    Place in: ServerScriptService/Core/GameCore.lua
    
    UPDATES:
    - Fixed pet movement system (no runtime script creation)
    - Enhanced proximity-based collection
    - Immediate pet selling with UI updates
    - Better sound handling with working IDs
...
GameCore.Systems = {
	Pets = {
		ActivePets = {}, -- [petInstance] = {owner, petId, data, connection}
		SpawnAreas = {},
		SpawnTimers = {},
		BehaviorConnections = {} -- Track all pet behavior connections
	},
	Shop = {
...
function GameCore:SetupEventHandlers()
	...
	-- Alternative pet collection handler
...
-- Pet System Implementation (UPDATED)
...
-- UPDATED: Enhanced pet model creation with movement
function GameCore:CreatePetModel(petConfig, position)
	...
	if template then
		...
		-- Ensure parts are set up for movement
		...
		-- Create enhanced fallback model with movement capability
	...
	-- Add movement and collection behaviors (FIXED - no script creation)
	self:StartPetBehavior(petModel, petConfig)

	return petModel
end

-- NEW: Enhanced pet model creation
function GameCore:CreateMovingPetModel(petConfig)
	local model = Instance.new("Model")
	model.Name = petConfig.name

	-- Create humanoid for movement
	...
	-- Main body part (HumanoidRootPart)
	...
	-- Add BodyVelocity for better movement control
...
-- NEW: Direct behavior management (no script creation)
-- Replace the StartPetBehavior function in GameCore.lua with this version:
...
function GameCore:StartPetBehavior(petModel, petConfig)
	...
	-- FIXED: Declare connection variable at the top
	local connection
	...
	-- Main behavior loop using RunService
	connection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		-- Check if humanoid/rootpart still exist
		...
		if math.random() < 0.01 then
			local lookAngle = math.random() * math.pi * 2
			local lookDirection = Vector3.new(math.cos(lookAngle), 0, math.sin(lookAngle))
			rootPart.CFrame = CFrame.lookAt(rootPart.Position, rootPart.Position + lookDirection)
		end

		-- Proximity detection for glow effect and collection
		local playerNearby = false
		local glowRadius = 8
		local collectRadius = 4
		...
		for _, player in pairs(Players:GetPlayers()) do
			if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
				...
				-- FIXED: Now connection is properly declared above
	...
	petModel:SetAttribute("BehaviorConnection", connection)
	self.Systems.Pets.BehaviorConnections[petModel] = connection

	-- Cleanup when pet is removed
	petModel.AncestryChanged:Connect(function()
		if not petModel.Parent then
			if connection then connection:Disconnect() end
			self.Systems.Pets.BehaviorConnections[petModel] = nil
			removeGlow()
		end
	end)
end
-- UPDATED: Enhanced wild pet collection
function GameCore:HandleWildPetCollection(player, petModel)
	if not player or not petModel or not petModel.Parent then 
		return false 
	end

	-- Validate player character proximity (server-side verification)
	...
	-- Verify distance (prevent exploiting)
	local distance = (playerRoot.Position - petPosition).Magnitude
	if distance > 6 then -- Slightly larger than client-side for lag compensation
	...
	-- Check if player can collect more pets
	local playerData = self:GetPlayerData(player)
	...
	self:CreatePetCollectionEffect(petModel, player)

	-- Clean up behavior connection
	local connection = self.Systems.Pets.BehaviorConnections[petModel]
	if connection then
		connection:Disconnect()
		self.Systems.Pets.BehaviorConnections[petModel] = nil
	end

	-- Delay destruction to allow effect to play
	spawn(function()
		wait(0.5)
	...
	if self.RemoteEvents.ShowNotification then
		self.RemoteEvents.ShowNotification:FireClient(player,
			"Pet Collected!", 
			"Caught " .. petData.name .. " (" .. petRarity .. ")!" .. 
				(rewards.coins > 0 and " +" .. rewards.coins .. " coins" or "") ..
				(rewards.gems > 0 and " +" .. rewards.gems .. " gems" or ""),
			"success"
		)
	end
...
function GameCore:CreatePetCollectionEffect(petModel, player)
	...
	if character and character:FindFirstChild("HumanoidRootPart") then
		...
		-- Create beam from pet to player
...
-- NEW: Calculate collection rewards
function GameCore:CalculateCollectionRewards(petConfig, rarity)
	...
	-- Chance for gems based on rarity
	local gemChances = {
		Common = 0.01,    -- 1%
		Uncommon = 0.05,  -- 5%
		Rare = 0.10,      -- 10%
	...
	if math.random() < gemChance then
		baseGems = math.ceil(mult / 4) -- Gems are rarer
	end
...
-- UPDATED: Enhanced pet selling with immediate feedback
...
function GameCore:CleanupPlayer(player)
	...
	for petInstance, connection in pairs(self.Systems.Pets.BehaviorConnections) do
		if connection then
			connection:Disconnect()
		end
	end
...
function GameCore:CleanupMemory()
	...
	if totalPets > 80 then
		for i = 1, math.min(#oldPets, 20) do
			if oldPets[i] and oldPets[i].Parent then
				-- Clean up behavior connection
				local connection = self.Systems.Pets.BehaviorConnections[oldPets[i]]
				if connection then
					connection:Disconnect()
					self.Systems.Pets.BehaviorConnections[oldPets[i]] = nil
				end
				oldPets[i]:Destroy()
			end
		end
	...
	for petInstance, connection in pairs(self.Systems.Pets.BehaviorConnections) do
		if not petInstance or not petInstance.Parent then
			if connection then connection:Disconnect() end
			self.Systems.Pets.BehaviorConnections[petInstance] = nil
		end
	end
...
function GameCore:ValidateAndFixPetModels()
	...
	if not petModelsFolder then
		...
		-- Create folder and basic models
	...
	for _, model in pairs(petModelsFolder:GetChildren()) do
		if model:IsA("Model") then
			...
			-- Check if parts are properly configured
			...
			if anchoredParts > 1 then -- Allow HumanoidRootPart to be anchored initially
...
function GameCore:FixPetModel(model)
	...
	if not rootPart then
		-- Try to use existing PrimaryPart
	...
	for _, part in pairs(model:GetDescendants()) do
		if part:IsA("BasePart") and part ~= rootPart then
			-- Unanchor all parts except root part
			part.Anchored = false
			part.CanCollide = false -- Prevent physics conflicts

			-- Ensure proper surface types
...
function GameCore:ValidateSystem()
	print("\n=== GAMECORE SYSTEM VALIDATION ===")

	-- Check essential components
	local checks = {
		["RemoteEvents"] = function() return self.RemoteEvents and next(self.RemoteEvents) end,
		["PetConfigs"] = function() return self.PetConfigs and next(self.PetConfigs) end,
		["SpawnAreas"] = function() return self.Systems.Pets.SpawnAreas and next(self.Systems.Pets.SpawnAreas) end,
		["DataStore"] = function() return self.DataStore or self.UseMemoryStore end
	}

	local allPassed = true
	for checkName, checkFunc in pairs(checks) do
		local success, result = pcall(checkFunc)
		if success and result then
			print("✅ " .. checkName .. " - OK")
		else
			print("❌ " .. checkName .. " - FAILED")
			allPassed = false
		end
	end
	...
	if allPassed then
		...
		warn("⚠️  Some system checks failed - review configuration")
	end
