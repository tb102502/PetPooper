...
-- FIXED Pet Model Creation in GameCore.lua
-- Replace your CreatePetModel function with this improved version
...
function GameCore:CreatePetModel(petConfig, position)
	...
	if template then
		...
		-- IMPORTANT: Ensure all parts are properly configured
		for _, part in pairs(petModel:GetDescendants()) do
			if part:IsA("BasePart") then
				...
				if part.Transparency >= 1 then
					part.Transparency = 0.5  -- Make semi-transparent if fully transparent
				end
			end
		end
	else
	...
	-- CRITICAL FIX: Proper positioning with validation
	local success = false

	-- Method 1: Try PrimaryPart positioning
	if petModel.PrimaryPart then
		petModel.PrimaryPart.CFrame = CFrame.new(position + Vector3.new(0, 5, 0)) -- Spawn 5 studs above ground
		success = true
		print("GameCore: Positioned " .. petConfig.name .. " using PrimaryPart at " .. tostring(position))

		-- Method 2: Try HumanoidRootPart positioning  
	elseif petModel:FindFirstChild("HumanoidRootPart") then
		petModel.HumanoidRootPart.CFrame = CFrame.new(position + Vector3.new(0, 5, 0))
		success = true
		print("GameCore: Positioned " .. petConfig.name .. " using HumanoidRootPart at " .. tostring(position))

		-- Method 3: Find any anchored part and position it
	else
		for _, part in pairs(petModel:GetDescendants()) do
			if part:IsA("BasePart") and part.Anchored then
				part.CFrame = CFrame.new(position + Vector3.new(0, 5, 0))
				success = true
				print("GameCore: Positioned " .. petConfig.name .. " using " .. part.Name .. " at " .. tostring(position))
				break
			end
		end
	end

	-- If positioning failed, try to fix the model structure
	if not success then
		warn("GameCore: Failed to position " .. petConfig.name .. " - attempting to fix structure")
		...
		if largestPart then
			petModel.PrimaryPart = largestPart
			largestPart.CFrame = CFrame.new(position + Vector3.new(0, 5, 0))
			success = true
			print("GameCore: Fixed structure and positioned " .. petConfig.name)
		end
	end

	-- Final validation - ensure the model is visible
	if success then
		-- Make sure model is in workspace
		petModel.Parent = workspace.Areas:FindFirstChild("Starter Meadow") and 
			workspace.Areas["Starter Meadow"]:FindFirstChild("Pets") or 
			workspace

		-- Double-check visibility
		spawn(function()
			wait(0.5) -- Small delay to ensure replication
			if petModel and petModel.Parent then
				...
				print("GameCore: Model " .. petModel.Name .. " has " .. partsCount .. " parts, " .. visibleParts .. " visible")

				if visibleParts == 0 then
					warn("GameCore: All parts invisible! Making them visible...")
					for _, part in pairs(petModel:GetDescendants()) do
						if part:IsA("BasePart") then
							part.Transparency = 0
						end
		...
		warn("GameCore: Failed to position pet model completely")
		petModel:Destroy()
		return nil
	end
