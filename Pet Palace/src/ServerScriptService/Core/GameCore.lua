...
function GameCore:Initialize()
	...
	self:SetupRemoteConnections()  -- Connect to remote events/functions
	self:SetupEventHandlers()      -- Set up the handlers
...
function GameCore:HandleItemSell(player, itemId, quantity)
    ...
    if itemId == "milk" then
        -- Check direct milk property first
        ...
        elseif playerData.inventory and playerData.inventory.milk then
            availableQuantity = playerData.inventory.milk
            itemLocation = "general"
        end
    else
        -- Check farming inventory for crops
        if playerData.farming and playerData.farming.inventory and playerData.farming.inventory[itemId] then
            availableQuantity = playerData.farming.inventory[itemId]
            itemLocation = "farming"
        -- Check general inventory for other items
        elseif playerData.inventory and playerData.inventory[itemId] then
            availableQuantity = playerData.inventory[itemId]
            itemLocation = "general"
        end
    end
    ...
    -- Get sell price
    local sellPrice = self:GetItemSellPrice(itemId) * quantity
    
    -- Remove items from inventory
    if itemId == "milk" then
        if itemLocation == "direct" then
            playerData.milk = math.max(0, playerData.milk - quantity)
        elseif itemLocation == "farming" then
            playerData.farming.inventory.milk = math.max(0, playerData.farming.inventory.milk - quantity)
        elseif itemLocation == "general" then
            playerData.inventory.milk = math.max(0, playerData.inventory.milk - quantity)
        end
    else
        if itemLocation == "farming" then
            playerData.farming.inventory[itemId] = math.max(0, playerData.farming.inventory[itemId] - quantity)
        elseif itemLocation == "general" then
            playerData.inventory[itemId] = math.max(0, playerData.inventory[itemId] - quantity)
        end
    end
    
    -- Add coins
    playerData.coins = (playerData.coins or 0) + sellPrice
    
    -- Update statistics
    playerData.totalEarned = (playerData.totalEarned or 0) + sellPrice
    
    -- Save data
    self:SavePlayerData(player, playerData)
    
    -- Send updated data to client
    self:SendPlayerDataUpdate(player)
    
    -- Notify player
    local itemName = self:GetItemDisplayName(itemId)
    self:ShowNotification(player, "💰 Item Sold!", 
        "Sold " .. quantity .. "x " .. itemName .. " for " .. sellPrice .. " coins!", "success")
    
    print("GameCore: Player " .. player.Name .. " sold " .. quantity .. "x " .. itemId .. " for " .. sellPrice .. " coins")
    return true, "Success"
end
-- Handle pig feeding
