...
function GameCore:HandleMilkCollection(player)
	...
	if currentTime - lastCollection < cooldown then
		...
		self:ShowNotification(player, "Cow Resting", 
			"The cow needs " .. math.ceil(timeLeft) .. " more seconds to produce milk!", "warning")
		return false
	end

	-- FIXED: Store milk in multiple locations for UIManager compatibility
	local milkAmount = ItemConfig.GetMilkAmount(playerData.upgrades or {})

	-- Method 1: Direct milk property (checked by UIManager)
	playerData.milk = (playerData.milk or 0) + milkAmount

	-- Method 2: In farming inventory (also checked by UIManager)
	if not playerData.farming then
		playerData.farming = {}
	end
	if not playerData.farming.inventory then
	...
	-- Method 3: In general inventory (backup check)
	if not playerData.inventory then
		playerData.inventory = {}
	end
	playerData.inventory.milk = (playerData.inventory.milk or 0) + milkAmount
