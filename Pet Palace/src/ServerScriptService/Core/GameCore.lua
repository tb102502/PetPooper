--[[
    GameCore.lua - CONSOLIDATED GAME CORE SYSTEM
    Replaces: PetSystemCore, ShopSystemCore, MainGameModule, and others
    Place in: ServerScriptService/Core/GameCore.lua
    
    This single module handles all core game functionality:
    - Pet System (collection, management, spawning)
    - Shop System (purchases, currency, items)  
    - Farming System (crops, feeding, plots)
...
-- Remote Events Setup (Consolidated)
function GameCore:SetupRemoteEvents()
	...
	local events = {
		...
		-- Farming System
		"PlantSeed", "HarvestCrop", "FeedPet",
		-- General
		"PlayerDataUpdated", "NotificationSent"
	}
...
function GameCore:CreatePetModel(petConfig, position)
	...
	local template = petModels and petModels:FindFirstChild(petConfig.modelName)
...
function GameCore:HandlePurchase(player, itemId, quantity)
	...
	if not playerData[currency] or playerData[currency] < totalCost then
		self.RemoteEvents.NotificationSent:FireClient(player, "Insufficient " .. item.currency, "error")
		return
	end
...
function GameCore:PlantSeed(player, plotId, seedId)
	...
	if not plot then
		self.RemoteEvents.NotificationSent:FireClient(player, "Invalid plot", "error")
		return
	end

	-- Check if plot is already planted
	if plot:GetAttribute("IsPlanted") then
		self.RemoteEvents.NotificationSent:FireClient(player, "Plot already planted", "error")
		return
	end
	...
	if seedCount <= 0 then
		self.RemoteEvents.NotificationSent:FireClient(player, "No seeds available", "error")
		return
	end
	...
	-- Update client
	self.RemoteEvents.PlayerDataUpdated:FireClient(player, playerData)
	self.RemoteEvents.NotificationSent:FireClient(player, "Seed planted!", "success")
...
function GameCore:HarvestCrop(player, plotId)
	...
	if not plot or not plot:GetAttribute("IsPlanted") then
		self.RemoteEvents.NotificationSent:FireClient(player, "Nothing to harvest", "error")
		return
	end

	local growthStage = plot:GetAttribute("GrowthStage") or 0
	if growthStage < 4 then
		self.RemoteEvents.NotificationSent:FireClient(player, "Crop not ready", "error")
		return
	end
	...
	self:UpdatePlayerLeaderstats(player)
	self.RemoteEvents.PlayerDataUpdated:FireClient(player, playerData)
	self.RemoteEvents.NotificationSent:FireClient(player, "Harvested " .. yieldAmount .. " crops!", "success")
...
function GameCore:EquipPet(player, petId)
	...
	if not petToEquip then
		self.RemoteEvents.NotificationSent:FireClient(player, "Pet not found", "error")
		return
	end
	...
	for _, equippedPet in ipairs(playerData.pets.equipped) do
		if equippedPet.id == petId then
			self.RemoteEvents.NotificationSent:FireClient(player, "Pet already equipped", "error")
			return
		end
	end
	...
	if #playerData.pets.equipped >= GameConfig.MaxEquippedPets then
		self.RemoteEvents.NotificationSent:FireClient(player, "Max pets equipped", "error")
		return
	end
