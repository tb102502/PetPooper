...
--[[
    FIXED ERROR HANDLER - NO STACK OVERFLOW
    Place this INSTEAD of the previous error fixes
    
    FIXES:
    ❌ REMOVED the recursive GetPlayerData wrapper that caused stack overflow
    ✅ Safe remote event creation
    ✅ ItemConfig validation
    ✅ Data structure validation
    ✅ Debugging helpers
]]

-- ========== FIX #1: ENSURE ALL REMOTE EVENTS EXIST ==========
...
local requiredRemoteEvents = {
	"CollectMilk", "FeedPig", "PurchaseItem", "ItemPurchased", "CurrencyUpdated",
	"PlantSeed", "HarvestCrop", "SellCrop", "SellMilk", "PlayerDataUpdated", 
	"ShowNotification", "PurchaseChicken", "FeedChicken", "CollectEgg", 
	"ChickenPlaced", "ChickenMoved", "FeedAllChickens", "FeedChickensWithType",
	"UsePesticide", "TeleportToCave", "TeleportToSurface", "SellOre", 
	"UpdateMiningData", "UnlockCave"
}
...
print("🔧 Creating missing remote events...")
for _, eventName in ipairs(requiredRemoteEvents) do
	EnsureRemoteEvent(eventName)
end

print("🔧 Creating missing remote functions...")
for _, funcName in ipairs(requiredRemoteFunctions) do
	EnsureRemoteFunction(funcName)
end

-- ========== FIX #2: VALIDATE ITEMCONFIG INTEGRITY ==========

local function ValidateItemConfig()
	local success, ItemConfig = pcall(function()
		return require(game:GetService("ReplicatedStorage"):WaitForChild("ItemConfig"))
	end)

	if not success then
		warn("❌ Failed to load ItemConfig: " .. tostring(ItemConfig))
		return false
	end

	local issues = {}

	-- Check if all required ore types exist
	local requiredOres = {"copper_ore", "bronze_ore", "iron_ore", "silver_ore", "gold_ore", "platinum_ore", "diamond_ore", "obsidian_ore"}
	for _, oreType in ipairs(requiredOres) do
		if not ItemConfig.MiningSystem or not ItemConfig.MiningSystem.ores or not ItemConfig.MiningSystem.ores[oreType] then
			table.insert(issues, "Missing ore type: " .. oreType)
		end
	end

	-- Check if all mega upgrades exist
	local requiredMegaUpgrades = {"mega_milk_boost", "mega_growth_speed", "mega_crop_multiplier", "mega_efficiency", "mega_golden_touch"}
	for _, upgradeId in ipairs(requiredMegaUpgrades) do
		if not ItemConfig.ShopItems or not ItemConfig.ShopItems[upgradeId] then
			table.insert(issues, "Missing mega upgrade: " .. upgradeId)
		end
	end

	-- Check milk efficiency chain
	local milkUpgrades = {"milk_efficiency_1", "milk_efficiency_2", "milk_efficiency_3"}
	for _, upgradeId in ipairs(milkUpgrades) do
		if not ItemConfig.ShopItems or not ItemConfig.ShopItems[upgradeId] then
			table.insert(issues, "Missing milk upgrade: " .. upgradeId)
		end
	end

	-- Check cave unlock items
	local caveUnlocks = {"cave_access_pass", "cave_2_unlock", "cave_3_unlock", "cave_4_unlock", "cave_5_unlock"}
	for _, unlockId in ipairs(caveUnlocks) do
		if not ItemConfig.ShopItems or not ItemConfig.ShopItems[unlockId] then
			table.insert(issues, "Missing cave unlock: " .. unlockId)
		end
	end

	if #issues > 0 then
		warn("❌ ItemConfig validation failed:")
		for _, issue in ipairs(issues) do
			warn("  " .. issue)
		end
		return false
	else
		print("✅ ItemConfig validation passed!")
		return true
	end
end

-- ========== FIX #3: SAFE PLAYER DATA STRUCTURE ==========

local function EnsurePlayerDataStructure(playerData)
	-- Don't call GameCore functions - just fix the data structure
	if not playerData then 
		playerData = {}
	end

	-- Ensure core currencies exist
	playerData.coins = playerData.coins or 100
	playerData.farmTokens = playerData.farmTokens or 0

	-- Ensure farming structure
	...
	-- Ensure defense structure (chickens)
...
-- ========== FIX #4: DEBUGGING HELPER FUNCTIONS ==========

local function DebugPlayerData(player)
	-- DON'T use GameCore:GetPlayerData here to avoid recursion
	print("=== DEBUG PLAYER DATA FOR " .. player.Name .. " ===")

	if _G.GameCore and _G.GameCore.PlayerData and _G.GameCore.PlayerData[player.UserId] then
		local playerData = _G.GameCore.PlayerData[player.UserId]
		print("✅ Player data exists in GameCore.PlayerData")
		print("Has farming:", playerData.farming ~= nil)
		print("Has livestock:", playerData.livestock ~= nil)
		print("Has defense:", playerData.defense ~= nil)
		print("Has mining:", playerData.mining ~= nil)
		print("Has upgrades:", playerData.upgrades ~= nil)
		...
		print("❌ Player data not found in GameCore.PlayerData")
		if _G.GameCore then
			print("GameCore exists:", true)
			print("PlayerData exists:", _G.GameCore.PlayerData ~= nil)
			if _G.GameCore.PlayerData then
				print("Player " .. player.UserId .. " in PlayerData:", _G.GameCore.PlayerData[player.UserId] ~= nil)
			end
		else
...
local function DebugRemoteEvents()
	...
	print("Total RemoteEvents: " .. eventCount)
	print("Total RemoteFunctions: " .. functionCount)
	print("===========================")
end

local function DebugGameCoreState()
	print("=== GAMECORE DEBUG ===")
	print("GameCore exists:", _G.GameCore ~= nil)

	if _G.GameCore then
		print("PlayerData exists:", _G.GameCore.PlayerData ~= nil)
		print("RemoteEvents exists:", _G.GameCore.RemoteEvents ~= nil)
		print("Systems exists:", _G.GameCore.Systems ~= nil)

		if _G.GameCore.PlayerData then
			local playerCount = 0
			for userId, data in pairs(_G.GameCore.PlayerData) do
		...
		if _G.GameCore.RemoteEvents then
			local remoteCount = 0
			for name, remote in pairs(_G.GameCore.RemoteEvents) do
				remoteCount = remoteCount + 1
			end
			print("Remote events:", remoteCount)
		end
	end
	print("=====================")
end

-- ========== FIX #5: SAFE INITIALIZATION ==========

local function SafeInitializePlayer(gameCore, player)
	-- Safely initialize player without calling potentially recursive functions
	local userId = player.UserId

	if not gameCore.PlayerData then
		gameCore.PlayerData = {}
	end

	if not gameCore.PlayerData[userId] then
		print("🔧 Creating default player data for " .. player.Name)
		gameCore.PlayerData[userId] = EnsurePlayerDataStructure({})
	else
		print("🔧 Fixing existing player data for " .. player.Name)
		gameCore.PlayerData[userId] = EnsurePlayerDataStructure(gameCore.PlayerData[userId])
	end

	return gameCore.PlayerData[userId]
end

-- ========== MAIN FIX FUNCTION ==========

local function ApplyStackOverflowFixes()
	print("🔧 Applying stack overflow fixes...")

	-- Validate ItemConfig
	local itemConfigValid = ValidateItemConfig()

	-- Fix player data for existing players
	if _G.GameCore and _G.GameCore.PlayerData then
		for userId, playerData in pairs(_G.GameCore.PlayerData) do
			_G.GameCore.PlayerData[userId] = EnsurePlayerDataStructure(playerData)
		end
		print("✅ Fixed existing player data structures")
	end

	-- Add safe initialization for new players
	if _G.GameCore then
		-- Store the original LoadPlayerData function
		if not _G.GameCore._OriginalLoadPlayerData then
			_G.GameCore._OriginalLoadPlayerData = _G.GameCore.LoadPlayerData
		end

		-- Replace with safer version
		_G.GameCore.LoadPlayerData = function(self, player)
			local success, result = pcall(function()
				return self:_OriginalLoadPlayerData(player)
			end)

			if success then
				return EnsurePlayerDataStructure(result)
			else
				warn("🔧 LoadPlayerData failed, using safe default: " .. tostring(result))
				return SafeInitializePlayer(self, player)
			end
		end

		print("✅ Wrapped LoadPlayerData with safe error handling")
	end

	print("✅ Stack overflow fixes applied!")
	return itemConfigValid
end

-- ========== EXPORT FUNCTIONS ==========

-- Make functions globally available for debugging
_G.DebugPlayerData = DebugPlayerData
_G.DebugRemoteEvents = DebugRemoteEvents
_G.DebugGameCoreState = DebugGameCoreState
_G.ValidateItemConfig = ValidateItemConfig
_G.ApplyStackOverflowFixes = ApplyStackOverflowFixes
_G.EnsurePlayerDataStructure = EnsurePlayerDataStructure

-- Auto-apply fixes when script loads
spawn(function()
	wait(2) -- Wait for other systems to load
	ApplyStackOverflowFixes()
end)

print("🔧 Stack overflow fixes loaded! Use these debug commands:")
print("  _G.DebugPlayerData(player) - Debug player data structure")  
print("  _G.DebugRemoteEvents() - Check remote events")
print("  _G.DebugGameCoreState() - Check GameCore state")
print("  _G.ValidateItemConfig() - Validate ItemConfig integrity")
print("  _G.ApplyStackOverflowFixes() - Manually apply fixes")
...
function GameCore:HandlePlotClick(player, spotModel)
	...
	remoteFolder.PlantSeed:FireClient(player, spotModel) -- Step 1: Client shows seed picker!
end

-- Called when client selects a seed for a plot
remoteFolder.PlantSeed.OnServerEvent:Connect(function(player, plotModel, seedId)
	print("GameCore: Received plant request from", player.Name, "for", seedId)
	pcall(function()
		GameCore:PlantSeed(player, plotModel, seedId)
	end)
end)
...
-- ADD this function to GameCore.lua (before Initialize function)
function GameCore:SetupRemoteConnections()
	print("GameCore: Setting up server-side remote connections...")

	local remoteFolder = ReplicatedStorage:FindFirstChild("GameRemotes")
	if not remoteFolder then
		warn("GameCore: GameRemotes folder not found!")
		return
	end
	...
	-- Get all remote events and functions
	for _, obj in ipairs(remoteFolder:GetChildren()) do
		if obj:IsA("RemoteEvent") then
			self.RemoteEvents[obj.Name] = obj
			print("GameCore: Connected RemoteEvent: " .. obj.Name)
		elseif obj:IsA("RemoteFunction") then
			self.RemoteFunctions[obj.Name] = obj
			print("GameCore: Connected RemoteFunction: " .. obj.Name)
		end
	end

	-- FORCE CREATE GetShopItems if missing
	if not self.RemoteFunctions.GetShopItems then
		print("🔧 GameCore: Creating missing GetShopItems RemoteFunction...")
		local getShopItemsRemote = Instance.new("RemoteFunction")
		getShopItemsRemote.Name = "GetShopItems"
		getShopItemsRemote.Parent = remoteFolder
		self.RemoteFunctions.GetShopItems = getShopItemsRemote
		print("✅ GameCore: Created GetShopItems RemoteFunction")
	end

	print("GameCore: Remote connections setup complete")
	print("  RemoteEvents: " .. self:CountTable(self.RemoteEvents))
	print("  RemoteFunctions: " .. self:CountTable(self.RemoteFunctions))
end

-- Helper function for counting tables
...
-- Replace your GameCore:Initialize() function with this fixed version
function GameCore:Initialize()
	...
	-- FIXED: Setup remote connections and handlers
	self:SetupRemoteConnections()  
--[[
    FIXED REMOTE EVENTS ORDER - Replace these functions in GameCore.lua
    
    The issue is the order of operations. We need to:
    1. Create missing remote events first
    2. Then load them into self.RemoteEvents
    3. Then set up the handlers
]]

	-- ========== 1. REPLACE YOUR EnsureSellingRemoteEvents FUNCTION ==========

	function GameCore:EnsureSellingRemoteEvents()
		print("GameCore: Ensuring selling remote events exist...")
		...
		-- Create SellItem event if it doesn't exist
		if not remoteFolder:FindFirstChild("SellItem") then
			local sellItemEvent = Instance.new("RemoteEvent")
			sellItemEvent.Name = "SellItem"
			sellItemEvent.Parent = remoteFolder
			print("✅ Created SellItem RemoteEvent")
		end

		-- Create ItemSold event for client feedback
		if not remoteFolder:FindFirstChild("ItemSold") then
			local itemSoldEvent = Instance.new("RemoteEvent")
			itemSoldEvent.Name = "ItemSold" 
			itemSoldEvent.Parent = remoteFolder
			print("✅ Created ItemSold RemoteEvent")
		end

		print("GameCore: Selling remote events created, refreshing connections...")

		-- ========== IMPORTANT: REFRESH REMOTE CONNECTIONS ==========
		-- Re-scan the GameRemotes folder to pick up newly created events
		for _, obj in ipairs(remoteFolder:GetChildren()) do
			if obj:IsA("RemoteEvent") then
				self.RemoteEvents[obj.Name] = obj
				print("GameCore: Loaded RemoteEvent: " .. obj.Name)
			elseif obj:IsA("RemoteFunction") then
				self.RemoteFunctions[obj.Name] = obj
				print("GameCore: Loaded RemoteFunction: " .. obj.Name)
			end
		end

		print("GameCore: Selling remote events ready and loaded")
	end

	-- ========== 2. REPLACE YOUR SetupEventHandlers FUNCTION ==========
	...
	function GameCore:SetupEventHandlers()
		...
		-- ========== SELLING SYSTEM EVENTS (FIXED WITH BETTER ERROR HANDLING) ==========

		-- Main item selling event
		if self.RemoteEvents.SellItem then
			self.RemoteEvents.SellItem.OnServerEvent:Connect(function(player, itemId, quantity)
				pcall(function()
					print("🛒 SERVER: Received sell request - " .. player.Name .. " wants to sell " .. (quantity or 1) .. "x " .. itemId)
					local success = self:HandleItemSell(player, itemId, quantity or 1)

					-- Send confirmation to client
					if success and self.RemoteEvents.ItemSold then
						local sellPrice, sellCurrency = self:GetItemSellPrice(itemId)
						if sellPrice then
							local totalEarnings = sellPrice * (quantity or 1)
							self.RemoteEvents.ItemSold:FireClient(player, itemId, quantity or 1, totalEarnings, sellCurrency)
						end
					end
				end)
			end)
			...
			warn("❌ SellItem remote event not found! Attempting to create it...")

			-- Try to create it directly if it's missing
			local remoteFolder = game:GetService("ReplicatedStorage"):FindFirstChild("GameRemotes")
			if remoteFolder then
				local sellItemEvent = Instance.new("RemoteEvent")
				sellItemEvent.Name = "SellItem"
				sellItemEvent.Parent = remoteFolder
				self.RemoteEvents.SellItem = sellItemEvent

				-- Connect the handler
				sellItemEvent.OnServerEvent:Connect(function(player, itemId, quantity)
					pcall(function()
						print("🛒 SERVER: Received sell request - " .. player.Name .. " wants to sell " .. (quantity or 1) .. "x " .. itemId)
						local success = self:HandleItemSell(player, itemId, quantity or 1)
				...
				print("✅ Created and connected SellItem event handler (fallback)")
			else
				error("GameCore: Could not create SellItem remote event - GameRemotes folder missing!")
			end
		end

		-- Legacy crop selling (for compatibility)
		if self.RemoteEvents.SellCrop then
			self.RemoteEvents.SellCrop.OnServerEvent:Connect(function(player, cropId, amount)
				pcall(function()
					self:HandleItemSell(player, cropId, amount or 1)
				end)
			end)
			print("✅ Connected SellCrop handler")
		end

		-- Enhanced milk selling
		if self.RemoteEvents.SellMilk then
			self.RemoteEvents.SellMilk.OnServerEvent:Connect(function(player, milkType, amount)
				pcall(function()
					...
					self:HandleItemSell(player, milkType, amount)
				end)
			end)
			print("✅ Connected SellMilk handler")
		end
		...
		if self.RemoteFunctions.GetShopItems then
			self.RemoteFunctions.GetShopItems.OnServerInvoke = function(player)
				print("🛒 SERVER: GetShopItems called by " .. player.Name)

				local success, result = pcall(function()
					-- Load ItemConfig safely
					local ItemConfig = require(ReplicatedStorage:WaitForChild("ItemConfig"))
					if not ItemConfig or not ItemConfig.ShopItems then
						error("ItemConfig.ShopItems not found")
					end

					local shopItemsArray = {}
					local itemCount = 0

					-- Convert dictionary to array with full data
					for itemId, item in pairs(ItemConfig.ShopItems) do
						itemCount = itemCount + 1

						-- Validate essential properties
						if not item.name or not item.price or not item.currency or not item.category then
							warn("🛒 SERVER: Item " .. itemId .. " missing essential properties - skipping")
							print("  name: " .. tostring(item.name))
							print("  price: " .. tostring(item.price))
							print("  currency: " .. tostring(item.currency))
							print("  category: " .. tostring(item.category))
						else
							-- Create complete item copy
							local itemCopy = {
								id = itemId,
								name = item.name,
								price = item.price,
					...
					print("🛒 SERVER: Processed " .. itemCount .. " items from ItemConfig")
					print("🛒 SERVER: Sending " .. #shopItemsArray .. " valid items to " .. player.Name)

					-- Debug: Show first few items
					print("🛒 SERVER: First 3 items preview:")
					for i = 1, math.min(3, #shopItemsArray) do
						local item = shopItemsArray[i]
						print("  " .. i .. ". " .. item.id .. " (" .. item.category .. ") - " .. item.price .. " " .. item.currency)
					end
				...
				if success then
					...
					warn("🛒 SERVER: GetShopItems failed: " .. tostring(result))
					-- Return empty array on failure
					return {}
				end
			end
			print("✅ Connected enhanced GetShopItems function")
		else
			warn("❌ GetShopItems RemoteFunction not available!")
		end

		-- ========== DEBUG: SHOW ALL CONNECTED EVENTS ==========
		print("GameCore: Event handlers setup complete!")
		print("📡 Available RemoteEvents:")
		for name, event in pairs(self.RemoteEvents) do
			print("  ✅ " .. name)
		end
		print("📡 Available RemoteFunctions:")
		for name, func in pairs(self.RemoteFunctions) do
			print("  ✅ " .. name)
		end
	end

	-- ========== 3. UPDATE YOUR INITIALIZE FUNCTION ORDER ==========
	-- REPLACE the Initialize function in GameCore.lua with this version:

	function GameCore:Initialize()
		print("GameCore: Starting complete initialization...")
		...
		-- STEP 1: Ensure all remote events/functions exist FIRST
		print("GameCore: Step 1 - Creating required remotes...")
		self:EnsureAllRemoteEventsExist()

		-- STEP 2: Setup remote connections (now they all exist)
		print("GameCore: Step 2 - Setting up remote connections...")
		self:SetupRemoteConnections()

		-- STEP 3: Setup event handlers (including GetShopItems)
		print("GameCore: Step 3 - Setting up event handlers...")
		self:SetupEventHandlers()

		-- STEP 4: Test the shop system immediately
		print("GameCore: Step 4 - Testing shop system...")
		self:TestShopSystem()

		-- STEP 5: Initialize other systems
		print("GameCore: Step 5 - Initializing other systems...")
		self:InitializeLivestockSystem()
		self:InitializeShopSystem()
		self:InitializeFarmingSystem()
		self:InitializePestAndChickenSystems()

		-- STEP 6: Start update loops
		print("GameCore: Step 6 - Starting update loops...")
		self:StartUpdateLoops()

		-- STEP 7: Setup admin commands
		print("GameCore: Step 7 - Setting up admin commands...")
		self:SetupFarmPlotTestingCommands()

		print("GameCore: ✅ Complete initialization finished!")
		return true
	end

	-- ADD this new function to GameCore.lua:
	function GameCore:EnsureAllRemoteEventsExist()
		local remoteFolder = ReplicatedStorage:FindFirstChild("GameRemotes")
		if not remoteFolder then
			remoteFolder = Instance.new("Folder")
			remoteFolder.Name = "GameRemotes"
		...
		local requiredFunctions = {"GetShopItems", "GetPlayerData", "GetFarmingData"}
		...
		for _, funcName in ipairs(requiredFunctions) do
			if not remoteFolder:FindFirstChild(funcName) then
				...
				print("GameCore: Created missing RemoteFunction: " .. funcName)
			end
		end

		-- Required RemoteEvents
		local requiredEvents = {
			"CollectMilk", "FeedPig", "PurchaseItem", "ItemPurchased", 
			"CurrencyUpdated", "PlantSeed", "HarvestCrop", "SellCrop", 
			"SellMilk", "PlayerDataUpdated", "ShowNotification", "SellItem"
		}
		...
		for _, eventName in ipairs(requiredEvents) do
			if not remoteFolder:FindFirstChild(eventName) then
				...
				print("GameCore: Created missing RemoteEvent: " .. eventName)
			end
		end

		print("GameCore: All required remotes ensured to exist")
	end

	-- ADD this new function to GameCore.lua:
	function GameCore:TestShopSystem()
		print("🧪 GameCore: Testing shop system...")

		-- Test 1: Check if ItemConfig loads
		local success, ItemConfig = pcall(function()
			return require(ReplicatedStorage:WaitForChild("ItemConfig"))
		end)

		if not success then
			error("🧪 GameCore: ItemConfig failed to load: " .. tostring(ItemConfig))
		end
		print("🧪 GameCore: ✅ ItemConfig loads successfully")

		-- Test 2: Check if ShopItems exists
		if not ItemConfig.ShopItems then
			error("🧪 GameCore: ItemConfig.ShopItems is missing!")
		end
		...
		print("🧪 GameCore: ✅ ItemConfig.ShopItems contains " .. itemCount .. " items")

		-- Test 3: Check if GetShopItems remote exists
		if not self.RemoteFunctions.GetShopItems then
			error("🧪 GameCore: GetShopItems RemoteFunction missing!")
		end
		print("🧪 GameCore: ✅ GetShopItems RemoteFunction exists")

		-- Test 4: Test the conversion logic
		local testItems = {}
		for itemId, item in pairs(ItemConfig.ShopItems) do
		...
		print("🧪 GameCore: ✅ Conversion logic works - " .. #testItems .. " valid items")
		...
		print("🧪 GameCore: ✅ Category distribution:")
		for category, count in pairs(categories) do
			print("  " .. category .. ": " .. count .. " items")
		end

		print("🧪 GameCore: Shop system test completed successfully!")
	end
	-- ========== 4. ADD DEBUG COMMAND TO CHECK REMOTE EVENTS ==========

	-- Add this to your admin commands section:
	Players.PlayerAdded:Connect(function(player)
		player.Chatted:Connect(function(message)
			if player.Name == "TommySalami311" then -- Replace with your username
				...
				if command == "/checkremotes" then
					print("=== REMOTE EVENTS DEBUG ===")
					print("RemoteEvents in GameCore:")
					for name, event in pairs(GameCore.RemoteEvents or {}) do
						print("  ✅ " .. name .. " (" .. event.ClassName .. ")")
					end

					print("RemoteEvents in ReplicatedStorage:")
					local remoteFolder = game:GetService("ReplicatedStorage"):FindFirstChild("GameRemotes")
					if remoteFolder then
						for _, child in pairs(remoteFolder:GetChildren()) do
							if child:IsA("RemoteEvent") then
					...
					print("SellItem specifically:")
					print("  In GameCore: " .. tostring(GameCore.RemoteEvents.SellItem ~= nil))
					if remoteFolder then
						print("  In ReplicatedStorage: " .. tostring(remoteFolder:FindFirstChild("SellItem") ~= nil))
					end
					print("===========================")

				elseif command == "/forcesellremote" then
					-- Force create the SellItem remote if it's missing
					local remoteFolder = game:GetService("ReplicatedStorage"):FindFirstChild("GameRemotes")
					if not remoteFolder then
						remoteFolder = Instance.new("Folder")
						remoteFolder.Name = "GameRemotes"
					...
					if not remoteFolder:FindFirstChild("SellItem") then
						...
						sellItemEvent.Name = "SellItem"
						sellItemEvent.Parent = remoteFolder
						GameCore.RemoteEvents.SellItem = sellItemEvent

						-- Connect handler
						sellItemEvent.OnServerEvent:Connect(function(clickPlayer, itemId, quantity)
							pcall(function()
								GameCore:HandleItemSell(clickPlayer, itemId, quantity or 1)
							end)
						end)

						print("🔧 Admin: Force created SellItem remote event")
						GameCore:SendNotification(player, "Remote Fixed", "SellItem remote event created!", "success")
					else
						print("🔧 Admin: SellItem remote already exists")
						GameCore:SendNotification(player, "Remote Exists", "SellItem remote already exists", "info")
					end
				end
			end
		end)
	...
	print("✅ Fixed Remote Events Order loaded!")
	print("🔧 The selling remote events will now be created and loaded properly")
	print("🧪 Debug commands: /checkremotes, /forcesellremote")
...
function GameCore:HandleMilkCollection(player)
	...
	-- Check cooldown using multiple sources
	local userId = player.UserId
	local lastCollection = 0

	-- Check GameCore cooldown system
	if self.Systems and self.Systems.Livestock and self.Systems.Livestock.CowCooldowns then
		lastCollection = math.max(lastCollection, self.Systems.Livestock.CowCooldowns[userId] or 0)
	end
	...
	-- Calculate cooldown (default 60 seconds, with upgrades reducing it)
	local baseCooldown = 60
	local cooldown = baseCooldown

	-- Try to get cooldown from ItemConfig if available
	local success, ItemConfig = pcall(function()
		return require(ReplicatedStorage:WaitForChild("ItemConfig"))
	end)

	if success and ItemConfig and ItemConfig.GetMilkCooldown then
		local upgradeCooldown = pcall(function()
			return ItemConfig.GetMilkCooldown(playerData.upgrades or {})
		end)
		if upgradeCooldown and type(upgradeCooldown) == "number" then
	...
	if timeSinceCollection < cooldown then
		...
		print("🥛 GameCore:", player.Name, "on cooldown for", math.ceil(timeLeft), "seconds")
		return false
	end

	-- Calculate milk amount (default 2, with upgrades increasing it)
	local milkAmount = 2

	-- Try to get milk amount from ItemConfig if available
	if success and ItemConfig and ItemConfig.GetMilkAmount then
		local upgradeMilkAmount = pcall(function()
			return ItemConfig.GetMilkAmount(playerData.upgrades or {})
		end)
		if upgradeMilkAmount and type(upgradeMilkAmount) == "number" then
			milkAmount = upgradeMilkAmount
		end
	end

	-- Store milk in multiple inventory locations for compatibility
	-- Method 1: Direct milk property (for older systems)
	playerData.milk = (playerData.milk or 0) + milkAmount

	-- Method 2: Livestock inventory (primary storage)
	playerData.livestock.inventory.milk = (playerData.livestock.inventory.milk or 0) + milkAmount

	-- Method 3: Farming inventory (backup for UI compatibility)
	if not playerData.farming then
		playerData.farming = {inventory = {}}
	end
	if not playerData.farming.inventory then
	...
	if not self.Systems.Livestock then
		self.Systems.Livestock = {CowCooldowns = {}}
	end
	if not self.Systems.Livestock.CowCooldowns then
		self.Systems.Livestock.CowCooldowns = {}
	end
	...
	if not saveSuccess then
		warn("🥛 GameCore: Failed to save player data for", player.Name)
	end

	-- Update leaderstats
	local leaderSuccess = pcall(function()
		self:UpdatePlayerLeaderstats(player)
	end)
	if not leaderSuccess then
		warn("🥛 GameCore: Failed to update leaderstats for", player.Name)
	end

	-- Send player data update
	if self.RemoteEvents and self.RemoteEvents.PlayerDataUpdated then
		local updateSuccess = pcall(function()
			self.RemoteEvents.PlayerDataUpdated:FireClient(player, playerData)
		end)
		if not updateSuccess then
			warn("🥛 GameCore: Failed to send player data update for", player.Name)
		end
	end
	...
	print("🥛 GameCore: Successfully processed milk collection for", player.Name, "- gave", milkAmount, "milk")
	return true
end

-- FIXED: Enhanced SendNotification function with better error handling
function GameCore:SendNotification(player, title, message, notificationType)
	notificationType = notificationType or "info"

	-- Try remote event first
	if self.RemoteEvents and self.RemoteEvents.ShowNotification then
		local success = pcall(function()
			self.RemoteEvents.ShowNotification:FireClient(player, title, message, notificationType)
		end)
		if success then
	...
	if remoteFolder then
		...
		if notificationRemote then
			...
			if success then
				print("🔔 GameCore: Sent notification via direct remote to", player.Name, "-", title)
				return
			end
		end
	end
	...
	print("🔔 NOTIFICATION for " .. player.Name .. ": [" .. notificationType:upper() .. "] " .. title .. " - " .. message)
end

-- ADD this debug function to GameCore.lua for testing:
function GameCore:DebugMilkSystem(player)
	...
	print("Cooldown systems:")
	local currentTime = os.time()
	if self.Systems and self.Systems.Livestock and self.Systems.Livestock.CowCooldowns then
		local lastCollection = self.Systems.Livestock.CowCooldowns[player.UserId] or 0
		local timeSince = currentTime - lastCollection
		print("  GameCore cooldown: " .. timeSince .. " seconds ago")
	else
		print("  GameCore cooldown system: NOT INITIALIZED")
	end

	print("Remote events:")
	print("  ShowNotification exists:", self.RemoteEvents and self.RemoteEvents.ShowNotification ~= nil)
	print("  PlayerDataUpdated exists:", self.RemoteEvents and self.RemoteEvents.PlayerDataUpdated ~= nil)

	print("============================================")
end


-- ========== 2. ADD SELL ITEM FUNCTIONALITY TO GAMECORE ==========




function GameCore:HandleItemSell(player, itemId, quantity)
	print("🛒 SERVER: Sell request - " .. player.Name .. " wants to sell " .. quantity .. "x " .. itemId)
	...
	local inventoryPath = nil

	-- Check multiple inventory locations
	...
	if itemId == "milk" or itemId == "fresh_milk" then
		if playerData.milk and playerData.milk > 0 then
			availableQuantity = playerData.milk
			itemLocation = "direct_milk"
		elseif playerData.livestock and playerData.livestock.inventory and playerData.livestock.inventory.milk then
			availableQuantity = playerData.livestock.inventory.milk
			itemLocation = "livestock"
			inventoryPath = {"livestock", "inventory"}
		elseif playerData.farming and playerData.farming.inventory and playerData.farming.inventory.milk then
			availableQuantity = playerData.farming.inventory.milk
			itemLocation = "farming"
			inventoryPath = {"farming", "inventory"}
		end
	else
		-- Check all inventory locations for other items
	...
	if availableQuantity < quantity then
		print("🛒 SERVER: Insufficient quantity - has " .. availableQuantity .. ", needs " .. quantity)
	...
	if not sellPrice or sellPrice <= 0 then
		print("🛒 SERVER: Item cannot be sold - " .. itemId)
	...
	if itemLocation == "direct_milk" then
	...
	self:SendNotification(player, "💰 Item Sold!", 
		"Sold " .. quantity .. "x " .. itemName .. " for " .. totalEarnings .. " " .. sellCurrency .. "!", "success")

	print("🛒 SERVER: Successfully sold " .. quantity .. "x " .. itemId .. " for " .. totalEarnings .. " " .. sellCurrency)
	return true
end

-- Get item sell price (add this if you don't have it)
function GameCore:GetItemSellPrice(itemId)
	-- Default prices for common items
	local defaultPrices = {
		-- Milk products
		milk = {price = 15, currency = "coins"},
		fresh_milk = {price = 15, currency = "coins"},

		-- Crops
		carrot = {price = 5, currency = "farmTokens"},
		corn = {price = 12, currency = "farmTokens"},
		strawberry = {price = 25, currency = "farmTokens"},
		golden_fruit = {price = 100, currency = "farmTokens"},

		-- Chicken products
		chicken_egg = {price = 5, currency = "coins"}
	}

	local defaultData = defaultPrices[itemId]
	if defaultData then
		return defaultData.price, defaultData.currency
	end

	return nil, nil
end

-- Get display name for items (add this if you don't have it)
function GameCore:GetItemDisplayName(itemId)
	...
	return displayNames[itemId] or itemId:gsub("_", " ")
end

print("✅ Simple Selling System Fix loaded!")
print("🔧 Make sure to call self:EnsureSellingRemoteEvents() in your Initialize function")
print("🧪 Test with admin command: /giveitems then /testsel carrot 1")
-- Handle pig feeding
	...
	Players.PlayerAdded:Connect(function(player)
		player.Chatted:Connect(function(message)
			...
			if player.Name == "TommySalami311" then -- Change this to your username
				...
				elseif command == "/forceshopfix" then
					-- Force immediate shop fix
					print("🔧 FORCE FIXING SHOP FOR " .. player.Name)

					-- Give player everything needed
					local playerData = GameCore:GetPlayerData(player)
					playerData.coins = 50000
					playerData.farmTokens = 1000

					-- Force client to refresh shop
					if GameCore.RemoteEvents.PlayerDataUpdated then
						GameCore.RemoteEvents.PlayerDataUpdated:FireClient(player, playerData)
					end

					GameCore:SendNotification(player, "Shop Force Fixed", "Shop should now show all items! Try opening it.", "success")

				elseif command == "/testshopremote" then
					-- Test the shop remote directly
					print("🧪 TESTING SHOP REMOTE...")

					if GameCore.RemoteFunctions.GetShopItems then
						local items = GameCore.RemoteFunctions.GetShopItems.OnServerInvoke(player)
						print("Remote returned:", type(items), "with", type(items) == "table" and #items or 0, "items")

						if type(items) == "table" and #items > 0 then
							print("First item category:", items[1].category)
							local categories = {}
							for _, item in ipairs(items) do
								categories[item.category] = (categories[item.category] or 0) + 1
							end
							--print("Categories:", table.concat(vim.tbl_keys(categories), ", "))
						end
					else
...
function GameCore:HarvestCrop(player, plotModel)
	...
	-- FIX: Check if crop is ready (stage 4 is fully grown, not 3)
	local growthStage = plotModel:GetAttribute("GrowthStage") or 0
	if growthStage < 4 then -- FIXED: Was >= 3, now < 4
		local timeRemaining = self:GetCropTimeRemaining(plotModel)
		self:SendNotification(player, "Not Ready", 
			"Crop is not ready for harvest yet! " .. timeRemaining .. " remaining.", "warning")
		return false
	end
	...
	-- Calculate pest damage impact on yield
	local pestDamage = 0
	if _G.PestSystem and _G.PestSystem.ActivePests and _G.PestSystem.ActivePests[plotModel] then
	...
	-- Restore visual indicator to green (available for planting)
...
-- ADD this new function to calculate remaining crop time:
function GameCore:GetCropTimeRemaining(plotModel)
	local plantedTime = plotModel:GetAttribute("PlantedTime") or 0
	local plantType = plotModel:GetAttribute("PlantType") or ""

	if plantedTime == 0 then return "Unknown time" end

	local ItemConfig = require(game:GetService("ReplicatedStorage"):WaitForChild("ItemConfig"))
	local seedData = ItemConfig.GetSeedData(plantType .. "_seeds") -- Convert crop back to seed
	if not seedData then return "Unknown time" end

	local growTime = seedData.growTime or 300 -- Default 5 minutes
	local elapsedTime = os.time() - plantedTime
	local remainingTime = growTime - elapsedTime
...
-- ADD this new HARVEST ALL function:
				...
				for _, spot in pairs(plantingSpots:GetChildren()) do
					if spot:IsA("Model") and spot.Name:find("PlantingSpot") then
						...
						if not isEmpty then
							...
							if growthStage >= 4 then -- Fully grown
...
-- ADD remote event handler for harvest all:
-- Add this to your SetupEventHandlers function in GameCore.lua:
...
game.Players.PlayerAdded:Connect(function(player)
	player.Chatted:Connect(function(message)
		...
		if player.Name == "TommySalami311" then
			...
			elseif command == "/debugshopsystem" then
				-- Complete shop system debug
				print("=== COMPLETE SHOP SYSTEM DEBUG ===")
				...
				print("1. ItemConfig Status:")
				print("   Loads successfully: " .. tostring(success))
				if success then
					local itemCount = 0
					local categoryCount = {}
					for itemId, item in pairs(ItemConfig.ShopItems) do
				...
				-- Test RemoteFunction
				print("2. RemoteFunction Status:")
				print("   GetShopItems exists: " .. tostring(GameCore.RemoteFunctions.GetShopItems ~= nil))
				if GameCore.RemoteFunctions.GetShopItems then
					print("   Handler assigned: " .. tostring(GameCore.RemoteFunctions.GetShopItems.OnServerInvoke ~= nil))
				end

				-- Test calling the function
				print("3. Function Call Test:")
				if GameCore.RemoteFunctions.GetShopItems and GameCore.RemoteFunctions.GetShopItems.OnServerInvoke then
					local testSuccess, testResult = pcall(function()
						return GameCore.RemoteFunctions.GetShopItems.OnServerInvoke(player)
					end)
					print("   Call successful: " .. tostring(testSuccess))
					if testSuccess then
						print("   Returned items: " .. #testResult)
					else
			...
			elseif command == "/forceshopfix" then
				-- Force complete shop system fix
				print("🔧 FORCING COMPLETE SHOP SYSTEM FIX...")

				-- Recreate the GetShopItems remote
				local remoteFolder = ReplicatedStorage:FindFirstChild("GameRemotes")
				if not remoteFolder then
					remoteFolder = Instance.new("Folder")
					remoteFolder.Name = "GameRemotes"
				...
				-- Remove old GetShopItems if it exists
				local oldRemote = remoteFolder:FindFirstChild("GetShopItems")
				if oldRemote then
					oldRemote:Destroy()
				end

				-- Create new GetShopItems
				local newRemote = Instance.new("RemoteFunction")
				newRemote.Name = "GetShopItems"
				newRemote.Parent = remoteFolder

				-- Update GameCore reference
				GameCore.RemoteFunctions.GetShopItems = newRemote

				-- Set up the handler
				newRemote.OnServerInvoke = function(testPlayer)
					print("🔧 NEW REMOTE: GetShopItems called by " .. testPlayer.Name)
					...
					print("🔧 NEW REMOTE: Returning " .. #shopItemsArray .. " items")
					return shopItemsArray
				end

				print("🔧 SHOP SYSTEM COMPLETELY REBUILT!")
				GameCore:SendNotification(player, "Shop Fixed", "Shop system has been completely rebuilt! Try opening the shop now.", "success")

			elseif command == "/testshopremote" then
				-- Test the shop remote directly
				print("🧪 TESTING SHOP REMOTE DIRECTLY...")

				local remoteFolder = ReplicatedStorage:FindFirstChild("GameRemotes")
				if remoteFolder then
					local getShopItemsRemote = remoteFolder:FindFirstChild("GetShopItems")
					if getShopItemsRemote then
						print("✅ GetShopItems remote found")
						...
						if success then
							...
							print("📦 Received " .. #result .. " items")
							print("First 3 items:")
							for i = 1, math.min(3, #result) do
								local item = result[i]
								print("  " .. i .. ". " .. item.id .. " (" .. item.category .. ")")
							end
						else
							print("❌ Remote call failed: " .. tostring(result))
						end
					else
						print("❌ GetShopItems remote not found")
					end
			...
			elseif command == "/clientshoptest" then
				-- Test client-side shop reception
				GameCore:SendNotification(player, "Client Test", "Check your client console for shop test results!", "info")

				-- This will fire a remote to test client-side
				if GameCore.RemoteEvents.ShowNotification then
					-- Use a custom remote for testing
					local testRemote = Instance.new("RemoteEvent")
					testRemote.Name = "TestClientShop"
					testRemote.Parent = ReplicatedStorage:FindFirstChild("GameRemotes")

					testRemote:FireClient(player)

					testRemote.Destroy(testRemote)
				end

				-- ADD this global debug function that can be called from Studio console:
				_G.DebugShopSystem = function()
					if _G.GameCore then
						_G.GameCore:TestShopSystem()
					else
						print("GameCore not available")
					end
				end

				_G.ForceShopFix = function()
					if _G.GameCore then
						print("🔧 Forcing shop system fix from console...")
						_G.GameCore:EnsureAllRemoteEventsExist()
						_G.GameCore:SetupRemoteConnections()
						_G.GameCore:SetupEventHandlers()
						_G.GameCore:TestShopSystem()
						print("🔧 Shop system fix complete!")
					else
						print("GameCore not available")
					end
				end

				print("🧪 Shop debug commands loaded!")
				print("Commands available:")
				print("  /debugshopsystem - Complete system debug")
				print("  /forceshopfix - Force rebuild shop system")
				print("  /testshopremote - Test remote function directly")
				print("  /clientshoptest - Test client-side reception")
				print("")
				print("Console commands:")
				print("  _G.DebugShopSystem() - Debug from console")
				print("  _G.ForceShopFix() - Fix from console")
