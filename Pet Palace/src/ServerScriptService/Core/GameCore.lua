...
function GameCore:HandlePurchase(player, itemId, quantity)
	...
	local itemConfig = self:GetShopItem(itemId)
	if not itemConfig then return false end

	local cost = (itemConfig.price or 0) * quantity
	local currency = itemConfig.currency or "coins"

	-- Check if player can afford
	if (playerData[currency] or 0) < cost then
		self:SendNotification(player, "Insufficient Funds", 
			"You need " .. cost .. " " .. currency .. " to buy this item!", "error")
		return false
	end
	...
	elseif itemConfig.type == "upgrade" then
		-- Handle upgrades
		playerData.upgrades = playerData.upgrades or {}
		playerData.upgrades[itemId] = (playerData.upgrades[itemId] or 0) + 1
		self:ApplyUpgradeEffect(player, itemId, playerData.upgrades[itemId])
	end

	-- Update leaderstats and UI immediately
	self:UpdatePlayerLeaderstats(player)

	-- Send updates to client
	if self.RemoteEvents.ItemPurchased then
		self.RemoteEvents.ItemPurchased:FireClient(player, itemId, quantity, cost, currency)
	end
	...
	-- DON'T save immediately - let batch system handle it
	-- This prevents the DataStore spam you're seeing
...
function GameCore:PlantSeed(player, plotModel, seedType)
	print("GameCore: PlantSeed called - Player:", player.Name, "Plot:", plotModel.Name, "Seed:", seedType)

	local playerData = self:GetPlayerData(player)
	if not playerData then 
		print("GameCore: No player data found")
		return false
	end

	-- Check farming data structure
	if not playerData.farming then
		print("GameCore: No farming data found")
		self:SendNotification(player, "No Farm Access", "You need to purchase a farm plot first!", "error")
		return false
	end

	if not playerData.farming.inventory then
		print("GameCore: No farming inventory found")
		self:SendNotification(player, "No Inventory", "Your farming inventory is not initialized!", "error")
		return false
	end

	-- Check if player has the seed
	local seedInventory = playerData.farming.inventory[seedType]
	if not seedInventory or seedInventory <= 0 then
		print("GameCore: Player doesn't have seed:", seedType, "Amount:", seedInventory)
		self:SendNotification(player, "No Seeds", "You don't have any " .. seedType:gsub("_", " ") .. "!", "error")
		return false
	end

	-- Check if plot is already planted
	if plotModel:GetAttribute("IsPlanted") then
		print("GameCore: Plot already planted")
		self:SendNotification(player, "Plot Occupied", "This plot already has something growing!", "warning")
		return false
	end

	-- Find an empty planting spot
	local plantingSpotsFolder = plotModel:FindFirstChild("PlantingSpots")
	if not plantingSpotsFolder then
		print("GameCore: No planting spots folder found")
		self:SendNotification(player, "Invalid Plot", "This farm plot is not properly configured!", "error")
		return false
	end

	local selectedSpot = nil
	for _, spotModel in pairs(plantingSpotsFolder:GetChildren()) do
		if spotModel:IsA("Model") and spotModel:GetAttribute("IsEmpty") == true then
	...
	if not selectedSpot then
		print("GameCore: No empty spots available")
		self:SendNotification(player, "Plot Full", "All planting spots in this plot are occupied!", "warning")
		return false
	end
	...
	if not seedConfig then
		print("GameCore: Seed config not found for:", seedType)
		self:SendNotification(player, "Invalid Seed", "Unknown seed type!", "error")
		return false
	end

	-- Remove seed from inventory
	playerData.farming.inventory[seedType] = playerData.farming.inventory[seedType] - 1
	print("GameCore: Removed 1", seedType, "from inventory. Remaining:", playerData.farming.inventory[seedType])

	-- Plant the seed in the selected spot
	selectedSpot:SetAttribute("IsEmpty", false)
	selectedSpot:SetAttribute("PlantType", seedType)
	selectedSpot:SetAttribute("GrowthStage", 1)
	selectedSpot:SetAttribute("PlantTime", os.time())
	selectedSpot:SetAttribute("TimeToGrow", seedConfig.growTime or 60)

	-- Update plot status
	plotModel:SetAttribute("IsPlanted", true)

	-- Create crop visual
	self:CreateCropInSpot(selectedSpot, seedType, 1)

	-- Start growth process
	self:StartCropGrowthInSpot(selectedSpot, seedConfig.growTime or 60)

	-- Save player data
	self:SavePlayerData(player)

	-- Send notifications
	local seedName = seedConfig.name or seedType:gsub("_", " ")
	self:SendNotification(player, "Seed Planted!", 
		"Planted " .. seedName .. " in spot " .. selectedSpot:GetAttribute("SpotNumber"), "success")
	...
	print("GameCore: Successfully planted", seedType, "in spot", selectedSpot:GetAttribute("SpotNumber"))
	return true
end

-- FIXED: Create crop visual in specific spot
function GameCore:CreateCropInSpot(spotModel, cropType, growthStage)
	...
	local spotPosition = plantingArea.Position
	...
	local baseSize = 1
	local sizeMultiplier = (growthStage / 4) * 0.8 + 0.2 -- Scale from 20% to 100%
	cropPart.Size = Vector3.new(baseSize * sizeMultiplier, baseSize * sizeMultiplier * 2, baseSize * sizeMultiplier)

	-- Position above the spot
	cropPart.Position = spotPosition + Vector3.new(0, cropPart.Size.Y/2 + 0.1, 0)

	-- Set appearance based on crop type
	if cropType == "carrot_seeds" then
	...
	if growthStage >= 4 then
		...
		-- Make the spot glow green when ready
...
function GameCore:StartCropGrowthInSpot(spotModel, totalGrowTime)
	...
	spawn(function()
		for stage = 2, 4 do
			...
			-- Check if spot still exists and has the same crop
			if spotModel and spotModel.Parent and spotModel:GetAttribute("PlantType") == cropType then
				...
				self:CreateCropInSpot(spotModel, cropType, stage)

				-- Update spot glow based on growth
				local glowBorder = spotModel:FindFirstChild("GlowBorder")
				if glowBorder then
					if stage >= 4 then
						-- Ready to harvest - bright green glow
						glowBorder.Color = Color3.fromRGB(0, 255, 0)
						glowBorder.Transparency = 0.3
					else
						-- Growing - yellow glow that gets brighter
						local intensity = stage / 4
