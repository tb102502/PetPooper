...
function GameCore:Initialize(shopSystem)
	...
	local success, dataStore = pcall(function()
		return game:GetService("DataStoreService"):GetDataStore("LivestockFarmData_v2") -- New version
	end)
...
function GameCore:InitializeEnhancedCowSystem()
	...
	self.CowPositions = {
		basePosition = Vector3.new(-320, -2, 120), -- Adjust to your map
...
function GameCore:PurchaseCow(player, cowType, upgradeFromCowId)
	print("🐄 GameCore: Processing cow purchase - " .. player.Name .. " buying " .. cowType)
	...
	if not cowConfig then
		self:SendNotification(player, "Invalid Cow", "Cow type not found!", "error")
		return false
	end
...
function GameCore:GetCowConfiguration(cowType)
	if not ItemConfig or not ItemConfig.ShopItems then return nil end

	local item = ItemConfig.ShopItems[cowType]
	if item and item.cowData then
		return item.cowData
	end
...
function GameCore:SavePlayerData(player, forceImmediate)
	...
	local success, errorMsg = pcall(function()
		self.PlayerDataStore:SetAsync("Player_" .. userId, {
			...
			boosters = playerData.boosters or {},
...
function GameCore:SetupAdminCommands()
	...
	local function setupAdminCommands()
		Players.PlayerAdded:Connect(function(player)
			player.Chatted:Connect(function(message)
				if player.Name == "YOUR_USERNAME_HERE" then -- CHANGE THIS TO YOUR USERNAME
					local args = string.split(message:lower(), " ")
					local command = args[1]

					if command == "/givecow" then
						-- Give player a basic cow
						local success = _G.GameCore:PurchaseCow(player, "basic_cow", nil)
						if success then
							print("✅ Admin: Gave basic cow to " .. player.Name)
						else
							print("❌ Admin: Failed to give cow to " .. player.Name)
						end

					elseif command == "/upgradecow" then
						local tier = args[2] or "silver"
						...
						if playerData and playerData.livestock and playerData.livestock.cows then
							for cowId, cowData in pairs(playerData.livestock.cows) do
								_G.GameCore:PurchaseCow(player, tier .. "_cow_upgrade", cowId)
								print("✅ Admin: Upgraded cow " .. cowId .. " to " .. tier)
								break -- Upgrade first cow found
							end
						end

					elseif command == "/givepasture" then
						local playerData = _G.GameCore:GetPlayerData(player)
						if playerData then
							playerData.upgrades = playerData.upgrades or {}
							playerData.upgrades.pasture_expansion_1 = true
							playerData.upgrades.pasture_expansion_2 = true
							playerData.upgrades.mega_pasture = true
							_G.GameCore:SavePlayerData(player)
							print("✅ Admin: Gave all pasture expansions to " .. player.Name)
						end

					elseif command == "/listcows" then
						local playerData = _G.GameCore:GetPlayerData(player)
						print("=== COWS FOR " .. player.Name .. " ===")

						if playerData and playerData.livestock and playerData.livestock.cows then
							for cowId, cowData in pairs(playerData.livestock.cows) do
								print("  " .. cowId .. ": " .. cowData.tier .. " tier")
								print("    Milk: " .. cowData.milkAmount .. " every " .. cowData.cooldown .. "s")
								print("    Total produced: " .. (cowData.totalMilkProduced or 0))
							end
						else
							print("  No cows found")
						end
					...
					elseif command == "/testvisuals" then
						local tier = args[2] or "rainbow"
					...
					elseif command == "/resetcows" then
						...
						if playerData and playerData.livestock then
							...
							print("✅ Admin: Reset all cows for " .. player.Name)
						end

					elseif command == "/cowhelp" then
						print("=== COW ADMIN COMMANDS ===")
						print("/givecow - Give yourself a basic cow")
						print("/upgradecow [tier] - Upgrade your first cow (silver/gold/diamond/rainbow/cosmic)")
						print("/givepasture - Give all pasture expansions")
						print("/listcows - List all your cows")
						print("/testvisuals [tier] - Test visual effects on your cow")
						print("/resetcows - Remove all your cows")
						print("/cowhelp - Show this help")
						print("========================")
					end
				end
			end)
	...
	Players.PlayerAdded:Connect(function(player)
		player.Chatted:Connect(function(message)
			...
			if player.Name == "TommySalami311" then
				...
				elseif command == "/giveallseeds" then
					local playerData = self:GetPlayerData(player)
					if playerData then
						if not playerData.farming then
						...
						-- Give all seeds from ItemConfig
						for itemId, itemData in pairs(ItemConfig.ShopItems) do
							if itemData.type == "seed" then
								playerData.farming.inventory[itemId] = 10
							end
						end

						self:SavePlayerData(player)
						self:SendNotification(player, "Admin: All Seeds Given", 
				...
				elseif command == "/testrarities" then
					-- Test all rarity levels
					local rarities = {"common", "uncommon", "rare", "epic", "legendary"}
					for i, rarity in ipairs(rarities) do
						local color = ItemConfig.GetRarityColor(rarity)
						local size = ItemConfig.GetRaritySize(rarity)

						local testPart = Instance.new("Part")
						testPart.Name = "RarityTest_" .. rarity
						testPart.Size = Vector3.new(2 * size, 2 * size, 2 * size)
						testPart.Color = color
						testPart.Material = Enum.Material.Neon
						testPart.Anchored = true
						testPart.Position = player.Character.HumanoidRootPart.Position + Vector3.new(i * 5, 5, 0)
					...
					print("Admin: Created rarity test display")
				...
				elseif command == "/checkshopitems" then
					local count = 0
					local categories = {}
					...
					print("Categories:")
					for cat, cnt in pairs(categories) do
						print("  " .. cat .. ": " .. cnt)
					end
					print("=======================")
				end
	...
	print("GameCore: Enhanced admin commands setup complete")
end
