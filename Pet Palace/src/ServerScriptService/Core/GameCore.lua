...
GameCore.SAVE_COOLDOWN = 60 -- Increased from 30 to reduce requests
GameCore.MAX_RETRIES = 3
GameCore.RETRY_DELAY = 2
GameCore.STUDIO_MODE = game:GetService("RunService"):IsStudio()
GameCore.DataStoreQueue = {
	pendingRequests = {},
	isProcessing = false,
	maxConcurrent = 1 -- Only 1 concurrent request in Studio
}
...
function GameCore:Initialize(shopSystem)
	print("GameCore: Starting enhanced initialization with DataStore fixes...")
	...
	-- Enable Studio development mode if in Studio
	if self.STUDIO_MODE then
		self:EnableStudioDevMode()
	end
	...
	-- Start enhanced update loops
	self:StartEnhancedUpdateLoops()

	-- Setup admin commands
	self:SetupEnhancedAdminCommands()

	print("GameCore: ✅ Enhanced initialization complete with DataStore fixes!")
	return true
end
function GameCore:InitializeDataStore()
	if self.STUDIO_MODE then
		print("🧪 GameCore: Running in Studio - Using enhanced DataStore handling")
		self.SAVE_COOLDOWN = 120 -- Even longer cooldown in Studio
		self.DataStoreQueue.maxConcurrent = 1
	else
		print("🌐 GameCore: Running in Live Game - Using normal DataStore handling")
	end

	local success, dataStore = pcall(function()
		return DataStoreService:GetDataStore("LivestockFarmData_v3") -- Changed version to avoid conflicts
	end)
...
-- ========== THROTTLED DATASTORE OPERATIONS ==========

function GameCore:QueueDataStoreRequest(requestType, key, data, callback)
	local request = {
		type = requestType, -- "GET" or "SET"
		key = key,
		data = data,
	...
	table.insert(self.DataStoreQueue.pendingRequests, request)

	if not self.DataStoreQueue.isProcessing then
		self:ProcessDataStoreQueue()
	end
end

function GameCore:ProcessDataStoreQueue()
	if self.DataStoreQueue.isProcessing then return end

	self.DataStoreQueue.isProcessing = true

	spawn(function()
		while #self.DataStoreQueue.pendingRequests > 0 do
			local request = table.remove(self.DataStoreQueue.pendingRequests, 1)

			if request then
				self:ExecuteDataStoreRequest(request)

				-- Delay between requests (especially important in Studio)
				local delay = self.STUDIO_MODE and 2 or 0.5
				wait(delay)
			end
		end

		self.DataStoreQueue.isProcessing = false
	end)
end

function GameCore:ExecuteDataStoreRequest(request)
	if not self.PlayerDataStore then
		warn("DataStore not available for request: " .. request.type)
		request.callback(false, "DataStore not available")
		return
	end

	local success, result = pcall(function()
		if request.type == "GET" then
			return self.PlayerDataStore:GetAsync(request.key)
		elseif request.type == "SET" then
			return self.PlayerDataStore:SetAsync(request.key, request.data)
		end
	end)

	if success then
		print("✅ DataStore " .. request.type .. " successful for key: " .. request.key)
		request.callback(true, result)
	else
		local errorMsg = tostring(result)
		warn("❌ DataStore " .. request.type .. " failed for key: " .. request.key .. " - " .. errorMsg)

		-- Retry logic for certain errors
		if request.retries < self.MAX_RETRIES and self:ShouldRetryDataStoreRequest(errorMsg) then
			request.retries = request.retries + 1
			print("🔄 Retrying DataStore request (attempt " .. request.retries .. "/" .. self.MAX_RETRIES .. ")")

			-- Re-queue with delay
			wait(self.RETRY_DELAY * request.retries)
			table.insert(self.DataStoreQueue.pendingRequests, request)
		else
			print("💀 DataStore request failed permanently after " .. request.retries .. " retries")
			request.callback(false, errorMsg)
		end
	end
end

function GameCore:ShouldRetryDataStoreRequest(errorMsg)
	...
	for _, retryableError in ipairs(retryableErrors) do
		if errorMsg:find(retryableError) then
			return true
		end
	end
...
function GameCore:SavePlayerData(player, forceImmediate)
	...
	if not forceImmediate then
		...
		if timeSinceLastSave < self.SAVE_COOLDOWN then
			print("⏰ Save skipped for " .. player.Name .. " (cooldown: " .. (self.SAVE_COOLDOWN - timeSinceLastSave) .. "s remaining)")
			return false
		end
	end
	...
	local safeData = self:SanitizeDataForSaving(playerData)
	if not safeData then
		warn("Data sanitization failed for " .. player.Name)
		return false
	end

	print("💾 Queuing save for " .. player.Name .. " (size: " .. self:EstimateDataSize(safeData) .. ")")

	-- Queue the save request instead of executing immediately
	self:QueueDataStoreRequest("SET", "Player_" .. userId, safeData, function(success, result)
		if success then
...
function GameCore:LoadPlayerData(player)
	...
	if not self.PlayerDataStore then
		...
		self:InitializePlayerSystems(player, defaultData)
	...
	self:QueueDataStoreRequest("GET", "Player_" .. userId, nil, function(success, data)
		...
		if success and data then
			...
			-- Convert saved cow position data back to Vector3
	...
	-- Return default data immediately for now (will be updated when load completes)
	self.PlayerData[userId] = defaultData
	return defaultData
end

-- ========== REDUCED AUTO-SAVE SYSTEM ==========

function GameCore:StartEnhancedUpdateLoops()
	print("GameCore: Starting enhanced update loops with DataStore throttling...")

	-- Enhanced auto-save loop with better throttling
	spawn(function()
		while true do
			if self.STUDIO_MODE then
				wait(600) -- 10 minutes in Studio
			else
				wait(300) -- 5 minutes in live game
			end

			print("🔄 Starting scheduled auto-save...")
			local playerCount = 0
			local saveCount = 0
			...
			for _, player in ipairs(Players:GetPlayers()) do
				if player and player.Parent and self.PlayerData[player.UserId] then
					...
					-- Check if player actually needs saving
					if self:PlayerNeedsSaving(player) then
						self:SavePlayerData(player)
						saveCount = saveCount + 1

						-- Add delay between saves in Studio
						if self.STUDIO_MODE and saveCount > 0 then
							wait(2)
						end
					end
				end
			...
			print("📊 Auto-save complete: " .. saveCount .. "/" .. playerCount .. " players saved")
		end
	end)
...
function GameCore:PlayerNeedsSaving(player)
	local playerData = self.PlayerData[player.UserId]
	if not playerData then return false end

	local lastSave = self.DataStoreCooldowns[player.UserId] or 0
	local timeSinceLastSave = os.time() - lastSave

	-- Only save if it's been a while or player has significant changes
	return timeSinceLastSave > self.SAVE_COOLDOWN
end

function GameCore:EnableStudioDevMode()
	if not self.STUDIO_MODE then return end

	print("🧪 Enabling Studio Development Mode...")

	-- Reduce DataStore usage to minimum
	self.SAVE_COOLDOWN = 300 -- 5 minutes
	self.DataStoreQueue.maxConcurrent = 1

	-- Override save method to be less aggressive
	local originalSavePlayerData = self.SavePlayerData
	self.SavePlayerData = function(self, player, forceImmediate)
		if not forceImmediate then
			print("📝 Studio Dev Mode: Skipping automatic save for " .. player.Name)
			return true
		end
		return originalSavePlayerData(self, player, forceImmediate)
	end

	print("✅ Studio Development Mode enabled - DataStore usage minimized")
end
function GameCore:ForcePlayerSave(player)
	print("💾 Force saving player: " .. player.Name)
	return self:SavePlayerData(player, true)
end

function GameCore:SaveAllPlayers()
	print("💾 Force saving all players...")
	local count = 0
	...
	for _, player in ipairs(Players:GetPlayers()) do
		if player and player.Parent and self.PlayerData[player.UserId] then
			...
			if count > 1 then wait(1) end
		end
	end

	print("✅ Initiated save for " .. count .. " players")
end

function GameCore:GetDataStoreStatus()
	local status = {
		connected = self.PlayerDataStore ~= nil,
		studioMode = self.STUDIO_MODE,
		queueSize = #self.DataStoreQueue.pendingRequests,
		isProcessing = self.DataStoreQueue.isProcessing,
		saveCooldown = self.SAVE_COOLDOWN,
...
function GameCore:PrintDataStoreStatus()
	local status = self:GetDataStoreStatus()

	print("=== DATASTORE STATUS ===")
	print("Connected: " .. tostring(status.connected))
	print("Studio Mode: " .. tostring(status.studioMode))
	print("Queue Size: " .. status.queueSize)
	print("Processing: " .. tostring(status.isProcessing))
	print("Save Cooldown: " .. status.saveCooldown .. "s")
	print("Max Retries: " .. status.maxRetries)
	print("========================")
end
...
function GameCore:SetupEnhancedAdminCommands()
		...
		player.Chatted:Connect(function(message)
			if player.Name == "TommySalami311" then -- Replace with your username
				...
				if command == "/datastatus" then
					self:PrintDataStoreStatus()

				elseif command == "/forcesave" then
					self:ForcePlayerSave(player)
				...
				elseif command == "/clearqueue" then
					self.DataStoreQueue.pendingRequests = {}
					print("DataStore queue cleared")

				elseif command == "/debugsavedata" then
					self:DebugPlayerDataForSaving(player)
				end
			end
		end)
	end)
...
print("🔧 DataStore Fixes Loaded!")
print("✅ Features:")
print("  📊 Request queue management")
print("  🔄 Automatic retry system") 
print("  ⏰ Enhanced cooldown system")
print("  🧪 Studio development mode")
print("  💾 Manual save commands")
print("")
print("🎮 Admin Commands:")
print("  /datastatus - Show DataStore status")
print("  /forcesave - Force save your data")
print("  /saveall - Force save all players")
print("  /devmode - Enable studio dev mode")
print("  /clearqueue - Clear DataStore queue")
