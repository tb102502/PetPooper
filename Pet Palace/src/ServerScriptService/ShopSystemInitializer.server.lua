	...
	if petSystem then
		...
		if petSystem.Remotes and petSystem.Remotes.Events and petSystem.Remotes.Events.PetAdded then
			petSystem.Remotes.Events.PetAdded.OnServerEvent:Connect(function(player, petId, petData)
				...
				if ShopSystemCore and typeof(ShopSystemCore.AddCurrency) == "function" then
					ShopSystemCore:AddCurrency(player, "Coins", 10)
				else
					warn("ShopSystemInitializer: AddCurrency method not available")
				end
			end)
		end
	else
...
-- Initialize the shop system with error handling
print("ShopSystemInitializer: Starting initialization...")

local initSuccess = pcall(function()
	if ShopSystemCore and typeof(ShopSystemCore.Initialize) == "function" then
		ShopSystemCore:Initialize()
		print("ShopSystemInitializer: Core system initialized successfully")
	else
		error("ShopSystemCore.Initialize method not found")
	end
end)

if not initSuccess then
	warn("ShopSystemInitializer: Failed to initialize core system")
	return
end

-- Connect to other systems
pcall(ConnectToExistingEvents)
pcall(SetupCurrencyEvents)
