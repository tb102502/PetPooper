...
local SimplifiedShopManager = ModuleLoader.LoadModule(script.Parent.Utility.SimplifiedShopManager)
...
function GuiManager.Initialize()
	...
	if SimplifiedShopManager then SimplifiedShopManager.Initialize() end
...
function GuiManager.OpenShop(tabName)
	if SimplifiedShopManager then
		SimplifiedShopManager.OpenShop(tabName)
	end
end
...
function GuiManager.UpdatePlayerData(newData)
	playerData = newData
	GuiManager.UpdateStats()
	GuiManager.UpdateCurrentTab()

	-- Update shop data if shop is loaded
	if SimplifiedShopManager then
		SimplifiedShopManager.UpdateShopData(newData.shopData)
	end
end
